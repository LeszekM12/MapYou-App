...
class App {
    constructor() {
        ...
        _App_createActive.set(this, false);
        _App_createWaypoints.set(this, []);
        _App_createMarkers.set(this, []);
        _App_createLine.set(this, null);
        _App_createSport.set(this, 'running');
        _App_createDistanceKm.set(this, 0);
        _App_createCoords.set(this, []);
        _App_createClickHandler.set(this, null);
        ...
        _App_lastAnnouncedKm.set(this, 0);
        _App_wasAutoPaused.set(this, false);
        _App_lastLapCount.set(this, 0);
        ...
        _App_activeRoute.set(this, null);
        _App_unifiedMarkers.set(this, []);
        _App_refreshing.set(this, false);
        ...
        if (await hasGPSPermission()) {
            ...
            if (saved) {
                try {
                    ...
                    this._loadMap(coords, __classPrivateFieldGet(this, _App_mapZoomLevel, "f"));
    ...
    _recenterMapToGPS(coords) {
        __classPrivateFieldSet(this, _App_userCoords, coords, "f");
        if (__classPrivateFieldGet(this, _App_map, "f")) {
            __classPrivateFieldGet(this, _App_map, "f").setView(coords, __classPrivateFieldGet(this, _App_mapZoomLevel, "f"), { animate: true });
        }
    }
    ...
    _loadMap(coords, zoom) {
        ...
        if (__classPrivateFieldGet(this, _App_clusterEnabled, "f")) {
            ...
            __classPrivateFieldGet(this, _App_map, "f").on('moveend zoomend', () => {
    ...
    _toggleVoice() {
        ...
        if (next)
            this._speak('Komunikaty głosowe włączone.');
    }
    _speak(text) {
    ...
    _updateVoiceStats(lat, lng) {
        if (!this._isVoiceCuesOn() || !__classPrivateFieldGet(this, _App_trackingActive, "f"))
            return;
        ...
        if (km >= next) {
            ...
            const elapsedSec = (Date.now() - __classPrivateFieldGet(this, _App_voiceStartTime, "f")) / 1000;
    ...
    async _requestWakeLock() {
        if (!this._isScreenLockOn())
            return; // respect Screen lock setting
      ...
      <div class="workout-modal__box">
        ...
        document.getElementById('workoutModalForm').addEventListener('submit', e => {
            ...
            const _wmEnriched = {
                ...
                coords: Array.isArray(_wm.routeCoords) && _wm.routeCoords.length > 0
                    ? _wm.routeCoords
                    : (Array.isArray(_wm.coords) && _wm.coords.length === 2
                        ? [_wm.coords] // single point [lat,lng] → wrap in array
                        : []),
            };
    ...
    _newWorkout(e) {
        ...
        const _enriched = {
            ...
            intensity: 0,
            notes: '',
            coords: Array.isArray(_w.routeCoords) && _w.routeCoords.length > 0
                ? _w.routeCoords
                : (Array.isArray(_w.coords) && _w.coords.length === 2
                    ? [_w.coords]
                    : []),
        };
    ...
    async _refreshClusterMarkers() {
        if (!__classPrivateFieldGet(this, _App_clusterEnabled, "f") || !__classPrivateFieldGet(this, _App_clusterGroup, "f"))
            return;
        if (__classPrivateFieldGet(this, _App_refreshing, "f"))
            return;
        ...
        try {
            ...
            if (__classPrivateFieldGet(this, _App_activeRoute, "f")) {
                ...
                __classPrivateFieldSet(this, _App_activeRoute, null, "f");
            }
            const { loadUnifiedWorkouts } = await import('./modules/db.js');
            const workouts = await loadUnifiedWorkouts();
            if (!workouts.length)
                return;
            const bounds = __classPrivateFieldGet(this, _App_map, "f").getBounds().pad(0.5);
            const seen = new Set();
            ...
            ${pace ? `<div class="wu-popup__stat">⚡ ${pace}</div>` : ''}
                ...
                marker.on('click', () => {
                    ...
                    if (w.coords.length > 1) {
                        const zoom = __classPrivateFieldGet(this, _App_map, "f").getZoom();
                        ...
                        __classPrivateFieldSet(this, _App_activeRoute, L.polyline(pts, {
                ...
                __classPrivateFieldGet(this, _App_clusterGroup, "f").addLayer(marker);
                __classPrivateFieldGet(this, _App_unifiedMarkers, "f").push(marker);
            });
        }
        finally {
            __classPrivateFieldSet(this, _App_refreshing, false, "f");
        }
    }
    ...
    _setLocalStorage() {
        ...
        if (last)
            void CS.saveWorkout(last.toJSON());
    }
    ...
    _refreshUnsavedBar() {
        ...
        const items = listPending('save-activity');
        if (!items.length)
            return;
      ...
      <div class="unsaved-list__sheet">
        ...
        <div class="unsaved-list__body">
          ${items.map(it => {
            ...
            const when = new Date(a.date ?? it.at).toLocaleString(undefined, { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' });
            return `
        ...
        ov.addEventListener('click', e => {
            ...
            if (row) {
                ov.remove();
                this._reopenPending(row.dataset.id);
            }
        });
    ...
    _initTracker() {
        ...
        __classPrivateFieldSet(this, _App_tracker, new Tracker(map, stats => {
            ...
            if (this._isVoiceCuesOn()) {
                ...
                if (km >= 1 && km > __classPrivateFieldGet(this, _App_lastAnnouncedKm, "f")) {
                    __classPrivateFieldSet(this, _App_lastAnnouncedKm, km, "f");
            ...
            if (exTime)
                exTime.textContent = formatDuration(stats.durationSec);
            if (exDist)
                exDist.textContent = formatDistance(stats.distanceKm);
            if (exPace)
                exPace.textContent = isCycling ? stats.speedKmH.toFixed(1) : formatPace(stats.paceMinKm);
            if (exPaceLbl)
                exPaceLbl.textContent = isCycling ? 'Avg speed (km/h)' : 'Avg pace (/km)';
            // Laps / splits — render when a new km completes
            const laps = stats.laps ?? [];
            if (laps.length !== __classPrivateFieldGet(this, _App_lastLapCount, "f")) {
                __classPrivateFieldSet(this, _App_lastLapCount, laps.length, "f");
            ...
            if (stats.autoPaused !== __classPrivateFieldGet(this, _App_wasAutoPaused, "f")) {
                __classPrivateFieldSet(this, _App_wasAutoPaused, !!stats.autoPaused, "f");
                const st = document.getElementById('trkStatus');
                if (st)
                    st.textContent = stats.autoPaused ? 'AUTO-PAUSED' : 'RECORDING...';
                const bar = document.getElementById('trkPausedBar');
                if (bar) {
                    bar.textContent = 'Auto-pause';
                    bar.classList.toggle('hidden', !stats.autoPaused);
                }
                ...
                if (exSt)
                    exSt.textContent = stats.autoPaused ? 'Auto-paused' : 'Recording';
                if (stats.autoPaused && this._isVoiceCuesOn())
                    this._speak('Automatyczna pauza.');
            }
        }), "f");
        // Historia
        ...
        document.getElementById('trkSportBtn')?.addEventListener('click', () => {
            if (__classPrivateFieldGet(this, _App_tracker, "f")?.isActive || __classPrivateFieldGet(this, _App_timerActive, "f"))
                return;
        ...
        document.getElementById('trkBtnStart')?.addEventListener('click', () => {
            ...
            if (isTrackable(sport)) {
                ...
                if (!__classPrivateFieldGet(this, _App_tracker, "f"))
                    return;
                __classPrivateFieldSet(this, _App_lastAnnouncedKm, 0, "f");
                __classPrivateFieldSet(this, _App_wasAutoPaused, false, "f");
                __classPrivateFieldSet(this, _App_lastLapCount, 0, "f");
                if (this._isAutoPauseOn())
                    void this._requestMotionPermission();
                __classPrivateFieldGet(this, _App_tracker, "f").setAutoPause(this._isAutoPauseOn());
    ...
    _isScreenLockOn() { return localStorage.getItem('mapyou_screen_lock') !== 'false'; } // default ON
    _isVoiceCuesOn() { return localStorage.getItem('mapyou_voice_cues') === 'true'; } // default OFF
    _isAutoPauseOn() { return localStorage.getItem('mapyou_auto_pause') === 'true'; } // default OFF
    ...
    _openTrackSettings() {
        ...
        const tile = (id, icon, label, sub, on) => `
          ...
          ${this._isDevMode() ? tile('simulate', '🧪', 'Simulate run', 'dev', false) : ''}
            ...
            overlay.querySelectorAll('[data-set]').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    if (id === 'auto_pause' && __classPrivateFieldGet(this, _App_tracker, "f"))
                        __classPrivateFieldGet(this, _App_tracker, "f").setAutoPause(!cur);
                    if (id === 'voice') {
    ...
    async _openRoutesScreen() {
      ...
      <div class="trk-routes-list" id="trkRoutesList"></div>
        ...
        const searchEl = overlay.querySelector('#trkRoutesSearch');
        let tab = 'saved';
        let community = [];
          ...
          <button class="trk-route-card__del" data-del="${r.id}" aria-label="Remove">✕</button>
            ...
            listEl.querySelectorAll('[data-share]').forEach(btn => {
                btn.addEventListener('click', e => {
                    ...
                    if (r)
                        this._openPublishDialog(r, async () => { await loadMyPublished(); renderSaved(searchEl.value); });
                });
            });
        ...
        <button class="trk-routes-seg__btn ${communityMode === 'featured' ? 'trk-routes-seg__btn--active' : ''}" data-cmode="featured">⭐ Featured</button>
            ...
            listEl.querySelectorAll('.trk-route-card').forEach(card => {
                card.addEventListener('click', () => {
                    ...
                    if (!r)
                        return;
                    const coords = decodePolyline(r.coordsEnc);
                    this._loadGhostRoute(coords, r.name || getSportLabel(r.sport));
                    overlay.remove();
                });
            });
            // Open the route owner's public profile (avatar / name)
            listEl.querySelectorAll('[data-owner]').forEach(el => el.addEventListener('click', e => {
                e.stopPropagation();
                const uid = el.dataset.owner;
                if (!uid)
                    return;
                overlay.remove();
                void import('./modules/PublicProfile.js').then(m => m.openPublicProfile(uid)).catch(() => { });
            }));
        };
        ...
        const switchTab = (t) => {
            tab = t;
            overlay.querySelectorAll('.trk-routes-tab').forEach(b => b.classList.toggle('trk-routes-tab--active', b.dataset.tab === t));
            searchEl.placeholder = t === 'saved' ? 'Search saved routes' : 'Search community routes';
            if (t === 'saved')
                renderSaved(searchEl.value);
            else if (communityLoaded)
                renderCommunity(searchEl.value);
            else
                void loadCommunity();
        };
        overlay.querySelectorAll('.trk-routes-tab').forEach(b => b.addEventListener('click', () => {
            if (b.disabled)
                return;
    ...
    _confirmDeleteRoute(inCommunity, onConfirm) {
    ...
    _trimRouteEnds(coords, meters = 200) {
        if (coords.length < 4)
            return coords;
        const near = (a, b) => L.latLng(a[0], a[1]).distanceTo(L.latLng(b[0], b[1])) < meters;
        let s = 0, e = coords.length - 1;
        while (s < e && near(coords[0], coords[s]))
            s++;
        while (e > s && near(coords[coords.length - 1], coords[e]))
            e--;
        const out = coords.slice(s, e + 1);
        return out.length >= 2 ? out : coords;
    }
    _openPublishDialog(route, onPublished) {
      ...
      <div class="trk-publish__body">
        ...
        ov.addEventListener('click', e => { if (e.target === ov)
            ov.remove(); });
        const trimBtn = ov.querySelector('#trkPubTrim');
        trimBtn.addEventListener('click', () => trimBtn.setAttribute('aria-checked', trimBtn.getAttribute('aria-checked') === 'true' ? 'false' : 'true'));
        const goBtn = ov.querySelector('#trkPubGo');
        goBtn.addEventListener('click', async () => {
            goBtn.disabled = true;
            goBtn.textContent = 'Publishing…';
            const name = ov.querySelector('#trkPubName').value.trim();
    ...
    async _publishRoute(route, trim, name) {
        let coords = route.coords;
        if (trim)
            coords = this._trimRouteEnds(coords);
        if (coords.length < 2)
            return false;
    ...
    _createProfile() { return __classPrivateFieldGet(this, _App_createSport, "f") === 'cycling' ? 'cycling' : 'walking'; }
    _enterCreateMode() {
        if (!__classPrivateFieldGet(this, _App_map, "f"))
            return;
        __classPrivateFieldSet(this, _App_createActive, true, "f");
        __classPrivateFieldSet(this, _App_createWaypoints, [], "f");
        __classPrivateFieldSet(this, _App_createCoords, [], "f");
        __classPrivateFieldSet(this, _App_createDistanceKm, 0, "f");
        __classPrivateFieldSet(this, _App_createSport, __classPrivateFieldGet(this, _App_trackSport, "f") || 'running', "f");
        this._clearGhostRoute();
        ...
        __classPrivateFieldSet(this, _App_createClickHandler, (e) => this._createAddWaypoint(e.latlng.lat, e.latlng.lng), "f");
        __classPrivateFieldGet(this, _App_map, "f").on('click', __classPrivateFieldGet(this, _App_createClickHandler, "f"));
    }
    _exitCreateMode() {
        if (__classPrivateFieldGet(this, _App_map, "f") && __classPrivateFieldGet(this, _App_createClickHandler, "f"))
            __classPrivateFieldGet(this, _App_map, "f").off('click', __classPrivateFieldGet(this, _App_createClickHandler, "f"));
        __classPrivateFieldSet(this, _App_createClickHandler, null, "f");
        this._createClear();
        __classPrivateFieldSet(this, _App_createActive, false, "f");
        document.getElementById('trkCreateBar')?.classList.add('hidden');
        document.getElementById('trkBottom')?.classList.remove('hidden');
    }
    _createAddWaypoint(lat, lng) {
        if (!__classPrivateFieldGet(this, _App_map, "f"))
            return;
        __classPrivateFieldGet(this, _App_createWaypoints, "f").push([lat, lng]);
        const m = L.circleMarker([lat, lng], { radius: 6, color: '#fff', weight: 2, fillColor: '#ff5a1f', fillOpacity: 1 }).addTo(__classPrivateFieldGet(this, _App_map, "f"));
        __classPrivateFieldGet(this, _App_createMarkers, "f").push(m);
        void this._createReroute();
    }
    async _createReroute() {
        if (!__classPrivateFieldGet(this, _App_map, "f"))
            return;
        const wp = __classPrivateFieldGet(this, _App_createWaypoints, "f");
        if (__classPrivateFieldGet(this, _App_createLine, "f")) {
            __classPrivateFieldGet(this, _App_map, "f").removeLayer(__classPrivateFieldGet(this, _App_createLine, "f"));
            __classPrivateFieldSet(this, _App_createLine, null, "f");
        }
        if (wp.length < 2) {
            __classPrivateFieldSet(this, _App_createCoords, wp.slice(), "f");
            __classPrivateFieldSet(this, _App_createDistanceKm, 0, "f");
            this._updateCreateUI();
            return;
        }
        ...
        try {
            ...
            __classPrivateFieldSet(this, _App_createDistanceKm, route.distance / 1000, "f");
            ...
            __classPrivateFieldSet(this, _App_createCoords, wp.slice(), "f");
            __classPrivateFieldSet(this, _App_createDistanceKm, this._coordsDistanceKm(wp), "f");
            __classPrivateFieldSet(this, _App_createLine, L.polyline(wp, { color: '#ff5a1f', weight: 5, opacity: 0.95, dashArray: '6 8' }).addTo(__classPrivateFieldGet(this, _App_map, "f")), "f");
        }
        this._updateCreateUI();
    }
    _createUndo() {
        __classPrivateFieldGet(this, _App_createWaypoints, "f").pop();
        const m = __classPrivateFieldGet(this, _App_createMarkers, "f").pop();
        if (m && __classPrivateFieldGet(this, _App_map, "f"))
            __classPrivateFieldGet(this, _App_map, "f").removeLayer(m);
        void this._createReroute();
    }
    _createClear() {
        __classPrivateFieldGet(this, _App_createMarkers, "f").forEach(m => { try {
            __classPrivateFieldGet(this, _App_map, "f")?.removeLayer(m);
        }
        catch { /* ignore */ } });
        __classPrivateFieldSet(this, _App_createMarkers, [], "f");
        __classPrivateFieldSet(this, _App_createWaypoints, [], "f");
        if (__classPrivateFieldGet(this, _App_createLine, "f")) {
            try {
                __classPrivateFieldGet(this, _App_map, "f")?.removeLayer(__classPrivateFieldGet(this, _App_createLine, "f"));
    ...
    _updateCreateUI() {
        ...
        if (hint)
            hint.textContent = __classPrivateFieldGet(this, _App_createWaypoints, "f").length === 0
                ...
                : `${__classPrivateFieldGet(this, _App_createWaypoints, "f").length} point${__classPrivateFieldGet(this, _App_createWaypoints, "f").length === 1 ? '' : 's'} · tap to add more`;
    }
    _updateCreateSportBtn() {
        const ic = document.getElementById('trkCreateSportIcon');
        const lb = document.getElementById('trkCreateSportLabel');
        if (ic)
            ic.textContent = getIcon(__classPrivateFieldGet(this, _App_createSport, "f"));
        if (lb)
            lb.textContent = getSportLabel(__classPrivateFieldGet(this, _App_createSport, "f"));
    }
    _createSave() {
        if (__classPrivateFieldGet(this, _App_createCoords, "f").length < 2) {
            const hint = document.getElementById('trkCreateHint');
            if (hint)
                hint.textContent = 'Tap at least 2 points to draw a route first.';
            return;
        }
      ...
      <div class="trk-publish__body">
        <label class="trk-publish__label">Route name</label>
        <input class="trk-routes-search" id="trkCNName" value="${(getSportLabel(__classPrivateFieldGet(this, _App_createSport, "f")) + ' route').replace(/"/g, '&quot;')}" />
        <p class="trk-publish__note">${__classPrivateFieldGet(this, _App_createDistanceKm, "f").toFixed(2)} km · ${getSportLabel(__classPrivateFieldGet(this, _App_createSport, "f"))} · saved to your Saved routes.</p>
        ...
        ov.querySelector('#trkCNGo')?.addEventListener('click', () => {
            const name = ov.querySelector('#trkCNName').value.trim() || getSportLabel(__classPrivateFieldGet(this, _App_createSport, "f"));
            saveRoute({
                id: 'drawn_' + Date.now().toString(36),
                name,
                sport: __classPrivateFieldGet(this, _App_createSport, "f"),
                distanceKm: __classPrivateFieldGet(this, _App_createDistanceKm, "f"),
                durationSec: 0,
                date: new Date().toISOString(),
                coords: __classPrivateFieldGet(this, _App_createCoords, "f"),
            });
            ov.remove();
            this._exitCreateMode();
        });
    ...
    _loopPoints(start, radiusM, bearing, dir, N = 8) {
        const dLat = radiusM / 111320;
        const dLng = radiusM / (111320 * Math.cos(start[0] * Math.PI / 180));
        const center = [start[0] + dLat * Math.sin(bearing), start[1] + dLng * Math.cos(bearing)];
    ...
    async _autoLoopMapbox(start, km) {
        ...
        for (let iter = 0; iter < 4; iter++) {
            ...
            const actual = __classPrivateFieldGet(this, _App_createDistanceKm, "f");
            if (!actual || Math.abs(actual - km) / km < 0.1)
                break; // within 10% → good enough
    ...
    _setTrackSport(sport) {
        __classPrivateFieldSet(this, _App_trackSport, sport, "f");
        const emojiEl = document.getElementById('trkSportBtnEmoji');
        const labelEl = document.getElementById('trkSportBtnLabel');
        if (emojiEl)
            emojiEl.textContent = getIcon(sport);
        if (labelEl)
            labelEl.textContent = getSportLabel(sport);
        ...
        const spBtn = document.getElementById('trkSportBtn');
        if (spBtn)
            spBtn.style.setProperty('--trk-sport-color', color);
    }
    _startClock() {
        ...
        const tick = () => {
            if (__classPrivateFieldGet(this, _App_timerActive, "f"))
                return; // elapsed handled by timer loop
            const now = new Date();
            if (clockEl)
                clockEl.textContent = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
            if (labelEl)
                labelEl.textContent = 'Time of day';
        };
        tick();
        if (__classPrivateFieldGet(this, _App_clockInterval, "f"))
    ...
    _drawRoute() {
        ...
        routeResult.classList.add('hidden');
        if (__classPrivateFieldGet(this, _App_routeLine, "f")) {
            __classPrivateFieldGet(this, _App_map, "f").removeLayer(__classPrivateFieldGet(this, _App_routeLine, "f"));
            __classPrivateFieldSet(this, _App_routeLine, null, "f");
        }
        this._stopRouteProgress();
        const [aLat, aLng] = __classPrivateFieldGet(this, _App_routePointA, "f");
        ...
        fetch(url)
            ...
            __classPrivateFieldSet(this, _App_routeLine, L.polyline(latLngs, { color: '#00c46a', weight: 6, opacity: 0.85 }).addTo(__classPrivateFieldGet(this, _App_map, "f")), "f");
            __classPrivateFieldGet(this, _App_map, "f").fitBounds(__classPrivateFieldGet(this, _App_routeLine, "f").getBounds(), { padding: [40, 40] });
...
_a = App, _App_map = new WeakMap(), _App_ghostRoute = new WeakMap(), _App_createActive = new WeakMap(), _App_createWaypoints = new WeakMap(), _App_createMarkers = new WeakMap(), _App_createLine = new WeakMap(), _App_createSport = new WeakMap(), _App_createDistanceKm = new WeakMap(), _App_createCoords = new WeakMap(), _App_createClickHandler = new WeakMap(), _App_tileLayer = new WeakMap(), _App_mapZoomLevel = new WeakMap(), _App_mapEvent = new WeakMap(), _App_workouts = new WeakMap(), _App_routeMode = new WeakMap(), _App_routeStep = new WeakMap(), _App_routePointA = new WeakMap(), _App_routePointB = new WeakMap(), _App_routeLine = new WeakMap(), _App_routeMarkerA = new WeakMap(), _App_routeMarkerB = new WeakMap(), _App_routeActivityMode = new WeakMap(), _App_routeCoords = new WeakMap(), _App_routeTotalDist = new WeakMap(), _App_progressLine = new WeakMap(), _App_progressWatchId = new WeakMap(), _App_coveredUpToIndex = new WeakMap(), _App_arrivedShown = new WeakMap(), _App_nearDestCount = new WeakMap(), _App_voiceKmAnnounced = new WeakMap(), _App_voiceStartTime = new WeakMap(), _App_voiceDistCovered = new WeakMap(), _App_trackingActive = new WeakMap(), _App_watchId = new WeakMap(), _App_trackingMarker = new WeakMap(), _App_trackingCoords = new WeakMap(), _App_prevTrackingCoords = new WeakMap(), _App_userTouchingMap = new WeakMap(), _App_recenterTimer = new WeakMap(), _App_tracker = new WeakMap(), _App_trackSport = new WeakMap(), _App_timerActive = new WeakMap(), _App_timerPaused = new WeakMap(), _App_timerStartMs = new WeakMap(), _App_timerAccumSec = new WeakMap(), _App_timerInterval = new WeakMap(), _App_lastAnnouncedKm = new WeakMap(), _App_wasAutoPaused = new WeakMap(), _App_lastLapCount = new WeakMap(), _App_clockInterval = new WeakMap(), _App_historyPanel = new WeakMap(), _App_nightMode = new WeakMap(), _App_wakeLock = new WeakMap(), _App_deferredInstallPrompt = new WeakMap(), _App_markers = new WeakMap(), _App_clusterGroup = new WeakMap(), _App_clusterEnabled = new WeakMap(), _App_activeRoute = new WeakMap(), _App_unifiedMarkers = new WeakMap(), _App_refreshing = new WeakMap(), _App_poiMarkers = new WeakMap(), _App_userCoords = new WeakMap(), _App_autocompleteTimer = new WeakMap(), _App_filterDrag = new WeakMap(), _App_activitySpeeds = new WeakMap(), _App_activeWorkoutId = new WeakMap(), _App_workoutRouteLayer = new WeakMap(), _App_customFilters = new WeakMap(), _App_pinnedCoord = new WeakMap(), _App_goalKm = new WeakMap(), _App_goalTime = new WeakMap(), _App_goalCount = new WeakMap(), _App_statsExpanded = new WeakMap(), _App_statsWeekOffset = new WeakMap(), _App_statsSelectedDay = new WeakMap(), _App_statsPrevGoalReached = new WeakMap();
...
document.getElementById('settingSync')?.addEventListener('click', async () => {
    ...
    const sub = document.querySelector('#settingSync .settings-item__sub');
    const logs = [];
    ...
    console.error = (...a) => { logs.push('ERR: ' + a.join(' ')); origErr(...a); };
    if (sub)
    ...
    if (sub)
        sub.textContent = logs.some(l => l.includes('complete')) ? 'Sync complete!' : 'Sync failed — try again';
});
