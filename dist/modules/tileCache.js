...
async function writeTile(key, blob) {
    try {
        await tbl().put({ key, blob, lastUsed: Date.now() });
...
export function installTileCache() {
    ...
    if (!L?.TileLayer) {
        if (installTries++ < 100) {
            setTimeout(installTileCache, 50);
            return;
        }
    ...
    const patched = (url, opts) => new CachedLayer(url, opts);
