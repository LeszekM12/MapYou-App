...
export function goalLabel(t, v) {
    if (t === 'distance')
        return `${v} km`;
    if (t === 'duration')
        return v >= 60 ? `${Math.round(v / 60 * 10) / 10} h` : `${v} min`;
    return `${v} ${v === 1 ? 'workout' : 'workouts'}`;
}
export function progressLabel(t, v) {
    if (t === 'distance')
...
function timeLeft(endAt) {
    ...
    if (ms < 0)
        return `Ended ${new Date(endAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}`;
...
// ── API ──────────────────────────────────────────────────────────────────────
export async function fetchEvents(clubId, scope = 'active') {
    try {
...
async function joinEvent(eventId, leave = false) {
    try {
...
// ── Create event modal ───────────────────────────────────────────────────────
export function openCreateEventModal(clubId, onCreated) {
    ...
    m.querySelector('#evCreate')?.addEventListener('click', async () => {
        const err = m.querySelector('#evCreateErr');
        const title = m.querySelector('#evTitle').value.trim();
        const goalValue = Number(m.querySelector('#evGoalValue').value);
        const startStr = m.querySelector('#evStart').value;
        const endStr = m.querySelector('#evEnd').value;
        if (!title) {
            err.textContent = 'Give the challenge a title.';
            return;
        }
        if (!(goalValue > 0)) {
            err.textContent = 'Target must be greater than 0.';
            return;
        }
        if (!startStr || !endStr) {
        ...
        if (endAt <= startAt) {
            err.textContent = 'The end date must be after the start date.';
            return;
        }
        const btn = m.querySelector('#evCreate');
        btn.disabled = true;
        ...
        try {
            const r = await fetch(`${BACKEND_URL}/clubs/${encodeURIComponent(clubId)}/events`, {
                ...
                body: JSON.stringify({
                    ...
                    goalType: m.querySelector('#evGoalType').value,
                    goalValue,
                    sport: m.querySelector('#evSport').value || null,
                    startAt, endAt,
                }),
            });
            if (!r.ok)
...
export async function renderEventsSection(host, clubId, isMember) {
    ...
    host.querySelectorAll('.ev-card').forEach(el => {
...
export function openEventDetail(eventId, onChange) {
    ...
    const render = async () => {
        ...
        const card = m.querySelector('.ev-modal__card');
        if (!ev) {
            card.innerHTML = `<div class="ev-empty"><span>📡</span><p>Could not load</p></div>`;
            return;
        }
        const me = getUserId();
      ...
      <button class="ev-btn ev-btn--ghost" id="evClose">Close</button>`;
        ...
        card.querySelector('#evDelete')?.addEventListener('click', async () => {
            if (!confirm(`Delete „${ev.title}"?\n\nThis removes the challenge for everyone. Workouts are not affected.`))
                return;
            const b = card.querySelector('#evDelete');
            b.disabled = true;
            ...
            if (await deleteEvent(eventId)) {
                ...
                b.disabled = false;
                b.textContent = 'Delete challenge';
            }
        });
        card.querySelector('#evJoin')?.addEventListener('click', async () => {
            const b = card.querySelector('#evJoin');
            b.disabled = true;
            if (await joinEvent(eventId, joined)) {
                await render();
                onChange?.();
            }
            else
...
//# sourceMappingURL=clubEvents.js.map