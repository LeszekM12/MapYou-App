...
const SLOW_RETRY_MS = 6 * 60 * 60000;
// ── Co wolno kolejkowac ──────────────────────────────────────────────────────
...
function wolnoSkleic(url, method) {
...
export async function enqueue(url, method, headers, body) {
    ...
    if (wolnoSkleic(url, method)) {
        try {
            const czekajace = await tbl().toArray();
            const bliznjak = czekajace.find(i => i.url === url && i.method === method && i.body === body);
            if (bliznjak) {
...
export async function flush() {
    ...
    try {
        ...
        let awaria = 0;
        const now = Date.now();
        const gotowe = items.filter(i => i.attempts < MAX_ATTEMPTS && (!i.nextTryAt || now >= i.nextTryAt));
        ...
        for (const item of items) {
            ...
            try {
                ...
                const backoff = attempts >= MAX_ATTEMPTS
                    ? SLOW_RETRY_MS
                    : Math.min(60 * 60000, 20000 * 2 ** (attempts - 1));
                ...
                if (attempts === MAX_ATTEMPTS) {
                    console.warn(`[Outbox] ${MAX_ATTEMPTS} nieudanych prob: ${item.method} ${item.url}` +
...
async function reviveAll() {
    try {
        ...
        if (!wstrzymane.length)
            return;
        for (const i of wstrzymane)
            await tbl().update(i.id, { nextTryAt: undefined });
...
window.mapyouOutbox =
    async (purge = false) => {
        if (purge === 'retry') {
            ...
            const stuck = all.filter(i => i.attempts > 0 || (i.nextTryAt && Date.now() < i.nextTryAt));
