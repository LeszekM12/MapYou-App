...
export class FriendsView {
    ...
    init() {
        ...
        window.__openLive = (token, name) => this._openLiveView(token, name);
            ...
            list.querySelectorAll('[data-delete]').forEach(btn => {
                btn.addEventListener('click', async () => {
                    ...
                    if (friend?.friendUserId && myUserId) {
                        try {
                            const res = await fetch(`${BACKEND_URL}/users/${encodeURIComponent(myUserId)}/friends/${encodeURIComponent(friend.friendUserId)}`, { method: 'DELETE' });
                            if (!res.ok) {
    ...
    async _saveLiveTokenFromUrl(url) {
        let token = '';
        try {
        ...
        if (!friend) {
            ...
            if (ownerId)
                friend = friends.find(f => f.friendUserId === ownerId);
            if (friend) {
    ...
    async _handleLivePushUrl(url) {
        ...
        const friendsBtn = document.querySelector('.bottom-nav__item[data-tab="tabFriends"]');
        friendsBtn?.click();
        // Otwórz live mapę
        setTimeout(() => this._openLiveView(token, name), 300);
    }
    // ── Toast ──────────────────────────────────────────────────────────────────
    _copyOrShareLink(link, name) {
        if (navigator.share) {
