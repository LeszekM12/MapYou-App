...
export function kartaSzkieletu(zeZdjeciem = true) {
    return `
...
export function pokazSzkieletFeedu(el, ile = 3) {
    if (!el)
        return;
    zapewnijStyl();
...
export function pokazSzkieletListy(el, ile = 5) {
    if (!el)
        return;
    zapewnijStyl();
    el.setAttribute('aria-busy', 'true');
...
export function koniecSzkieletu(el) {
...
export function podepnijZastepnikiObrazkow() {
    if (podpiete)
        return;
    podpiete = true;
    zapewnijStyl();
    document.addEventListener('error', (e) => {
        ...
        if (!zrodlo || zrodlo === 'null' || zrodlo === 'undefined')
            return;
        if (img.dataset.fallbackRetry !== '1') {
            img.dataset.fallbackRetry = '1';
            setTimeout(() => {
                if (!img.isConnected || img.dataset.fallbackDone === '1')
                    return;
