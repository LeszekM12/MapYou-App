...
function wyslijRaz(url, metoda, wyslij) {
    if (metoda !== 'GET')
        return wyslij();
...
export function onSessionReady(fn) {
    if (_sessionReady) {
        fn();
        return;
    }
...
export function setSessionReady(ready) {
    ...
    if (ready && _readyWaiters.length) {
        ...
        waiting.forEach(fn => { try {
            fn();
        }
        catch (e) {
            console.warn('[authFetch] subskrybent sesji rzucil blad:', e);
        } });
    }
}
            ...
            if (url.includes('/feed/like') && body) {
                try {
                    ...
                    if (req.itemId) {
                        ...
                        return new Response(JSON.stringify({ status: 'ok', liked, count }), { status: 200, headers: { 'Content-Type': 'application/json' } });
                    }
                }
