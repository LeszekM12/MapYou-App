...
export class Tracker {
    constructor(map, onUpdate) {
        ...
        Object.defineProperty(this, "_visHooked", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        });
        Object.defineProperty(this, "_lastStateSave", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        }); // throttling zapisu stanu sesji
        Object.defineProperty(this, "_pendingDraw", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        }); // punkty do dorysowania po powrocie
        ...
        Object.defineProperty(this, "_apAnchor", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        }); // anchor for stationary detection (null-speed iOS path)
        // Per-km splits (laps)
    ...
    _flushPendingDraw() {
        if (!this._pendingDraw.length || !this.polyline) {
            this._pendingDraw = [];
            return;
        }
        ...
        if (last)
            this.dotMarker?.setLatLng(L.latLng(last[0], last[1]));
    }
    ...
    _installVisibilityRedraw() {
        if (this._visHooked)
            return;
        this._visHooked = true;
        document.addEventListener('visibilitychange', () => {
            if (!document.hidden)
                this._flushPendingDraw();
        });
    }
    _startGPS() {
    ...
    _onPosition(pos) {
        ...
        if (this._autoPauseOn && !this._useMotionAP) {
            ...
            if (spd != null && !Number.isNaN(spd) && spd >= 0) {
                ...
                if (!stationary)
                    this._apAnchor = [lat, lng]; // escaped → re-anchor
            }
            if (stationary) {
...
export function recomputeDistance(coords, fallback = 0) {
    if (coords.length < 2)
        return fallback;
    let total = 0;
