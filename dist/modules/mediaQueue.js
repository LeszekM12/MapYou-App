...
export async function enqueueMedia(blob, filename, userId, folder, publicId) {
    ...
    if (!data.byteLength)
        throw new Error('pusty plik — nie kolejkuje');
...
export async function failedMediaCount() {
    try {
        const all = await tbl().toArray();
        return all.filter(j => j.attempts >= MAX_ATTEMPTS).length;
    }
...
async function replacePlaceholder(placeholder, realUrl) {
    ...
    try {
        document.querySelectorAll('img, video')
            .forEach(el => {
            if (!el.getAttribute('src')?.includes(placeholder))
                return;
            el.src = realUrl;
            el.style.display = '';
        ...
        document.querySelectorAll('.media-pending').forEach(n => {
            if (!n.parentElement?.querySelector('img[src], video[src]'))
                n.remove();
        });
    }
