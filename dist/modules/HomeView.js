...
import { loadUnifiedWorkouts, isVerifiedWorkout } from './UnifiedWorkout.js';
...
function openCommentsView(card, actId) {
    ...
    list.addEventListener('click', e => {
        ...
        cur.text = text;
        cur.editedAt = new Date().toISOString();
        render(items);
    ...
    let attached = null;
    const fileEl = ov.querySelector('#cvFile');
    const attachEl = ov.querySelector('#cvAttach');
    const attachImg = ov.querySelector('#cvAttachImg');
...
async function _fillElevation(root, full, canPersist) {
    ...
    if (!slot)
        return;
    const coords = (Array.isArray(full.coords) ? full.coords : []);
    if (coords.length < 5)
        return;
    let series = (full.elevSeries ?? null);
    ...
    if (!series) {
        ...
        try {
            ...
            series = sm.map((e, i) => [Math.round(dist[i]), Math.round(e * 10) / 10]);
            let g = 0;
            for (let i = 1; i < sm.length; i++) {
                const dv = sm[i] - sm[i - 1];
                if (dv > 0)
                    g += dv;
            }
            gain = Math.round(g);
            if (canPersist) {
                try {
                    await CS.saveEnrichedActivity({ ...full, elevSeries: series, elevGain: gain });
                }
                catch { /* ignore */ }
            }
        }
    ...
    const X = (m) => padL + (m / dMax) * (W - padL - 4);
    const Y = (e) => padT + (1 - (e - lo) / Math.max(1, hi - lo)) * (H - padT - padB);
...
export async function openActivityDetail(act, isOwn, actId) {
    ...
    const wszystkie = [full.photoUrl, ...(full.photos ?? [])]
        .filter((u) => !!u);
    const kafel = (u) => {
    ...
    {
        const tor = ov.querySelector('#adGalTrack');
        const dots = ov.querySelector('#adGalDots');
        if (tor && dots) {
            const kropki = Array.from(dots.children);
    ...
    setTimeout(() => {
        ...
        if (routePts.length > 0) {
            ...
            if (pts.length === 1) {
                ...
                const step = (now) => {
                    if (!_adMap)
                        return; // detail closed mid-animation
    ...
    ov.querySelectorAll('.ad-footer .home-card__action').forEach(btn => {
        btn.addEventListener('click', e => {
            ...
            if (action === 'share') {
                void import('./ShareStudio.js').then(m => isOwn ? m.openShareStudio(full) : m.openShareActions(full));
            }
        });
    });
...
export function buildCard(act) {
    ...
    <div class="home-card__header">
      ...
      <div class="home-card__meta">
        ${act.authorName ? `<span class="home-card__author-name" style="font-size:1.2rem;font-weight:700;color:#fff;display:block;line-height:1.2">${esc(act.authorName)}</span>` : ''}
...
function _parseDeepLink(url) {
    const act = _parseActivityDeepLink(url);
    if (act)
        return { kind: 'activity', id: act.id, userId: act.userId };
...
if (typeof window !== 'undefined' && !window.__activityDeepLinkBound) {
    ...
    navigator.serviceWorker?.addEventListener('message', e => {
        if (e.data?.type === 'OPEN_ACTIVITY') {
            const t = _parseDeepLink(e.data.url);
            if (t)
                void _routeNotifTarget(t);
        }
    });
...
export class HomeView {
    constructor() {
        ...
        Object.defineProperty(this, "_miniObserver", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
         ...
         *  renderowaniu. */
        Object.defineProperty(this, "_onlinePodpiete", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        Object.defineProperty(this, "_exploreCacheRam", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        if (!this._miniObserver) {
            this._miniObserver = new IntersectionObserver(entries => {
                for (const e of entries) {
                    ...
                    const enc = el.dataset.minimapEnc;
                    const sp = el.dataset.minimapSport ?? 'running';
                    if (!enc)
                        continue;
                    try {
    ...
    async _openHealthImport() {
      ...
      <div class="rwp-sheet">
        ...
        ov.addEventListener('click', e => { if (e.target === ov)
            close(); });
        const list = ov.querySelector('#hiList');
        ...
        list.querySelectorAll('.rwp-item:not(.hi-done)').forEach(el => el.addEventListener('click', () => {
    ...
    _buildStreakWidget(activities) {
        ...
        for (const w of this._workouts) {
            const k = dayKey(typeof w.date === 'number' ? w.date : Date.parse(w.date));
            activeKeys.add(k);
            if (isVerifiedWorkout(w))
                verifiedKeys.add(k);
        }
          ...
          <p class="home-empty__sub">Finish a workout or tap + to create a post</p>`;
            ...
            const buildOne = (item, idx) => {
                ...
                if (item.kind === 'activity') {
                    ...
                    if (mapEl && coordsEnc) {
                        const sport = (item.data.sport ?? activities.find(a => a.id === actId)?.sport ?? 'running');
    ...
    _pasekOffline() {
        if (document.getElementById('mapyouOfflineBanner'))
            return;
    ...
    _podepnijAutoOdswiezanie(userId) {
        if (this._onlinePodpiete)
            return;
    ...
    async _odswiezDane() {
        ...
        if (this._feedSig(poz) !== this._feedSig(this._lastHomeFeed)) {
            this._lastHomeFeed = poz;
            this._repaintFeed?.(poz);
        }
    }
    async _loadExplore() {
        ...
        if (!this._exploreCacheRam) {
            const z = await cacheOdczytaj(`explore:${userId}`);
            if (z)
                this._exploreCacheRam = z.value;
        }
        if (this._exploreCacheRam?.length) {
            ...
            const lista = document.getElementById('homeFeedList');
            if (!lista?.querySelector('.home-card'))
                pokazSzkieletFeedu(lista, 3);
        }
    ...
    async _wgrajCacheFeedu(userId) {
        if (!userId)
            return;
        const [f, e] = await Promise.all([
            cacheOdczytaj(`feed:${userId}`),
            cacheOdczytaj(`explore:${userId}`),
        ]);
        if (f)
            this._feedCacheRam = f.value;
        if (e)
            this._exploreCacheRam = e.value;
    }
    ...
    async _loadFeedItemMeta(card, itemId, itemType, userId) {
        try {
            ...
            if (cr.ok) {
                ...
                if (list) {
                    list.innerHTML = cd.data.map(c => `<div class="ff-comment"><span class="ff-comment__author">${esc(c.authorName)}</span><span class="ff-comment__text">${esc(c.text)}</span></div>`).join('');
                }
