...
function _renderFull(overlay, sheet, profile, activities, posts, myUserId, hasReels = false, hasUnseen = false, targetReelGroup = null) {
  ...
  `;
    ...
    if (!content)
        return; // private profile — no tabs
    ...
    sheet.querySelectorAll('.pv-subtab').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (tab === 'activities')
                ...
                _renderTrophiesTab(content, activities, profile.weeklyWins ?? 0, profile.bestStreak ?? 0, profile.userId);
            else
...
function _renderTrophiesTab(el, activities, weeklyWins, bestStreak, userId) {
    el.innerHTML = '<div class="pv-empty"><p>Loading trophies…</p></div>';
    void (async () => {
        let items = [];
        try {
            const r = await fetch(`${BACKEND_URL}/achievements/user/${encodeURIComponent(userId)}`, { cache: 'no-store' });
    ...
    el.querySelectorAll('.pv-act-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            if (!a)
                return;
            const d = a.data;
            const isOwn = d.userId === myUserId;
            const actId = (d.activityId ?? d.id);
...
// ── Like/Comment helper ──────────────────────────────────────────────────────
function _actionsHtml(itemId, itemType, likeCount = 0, commentCount = 0) {
...
function _attachLikeComment(card, itemId, itemType) {
    const userId = localStorage.getItem('mapyou_userId_profile') ?? '';
    const userName = localStorage.getItem('mapyou_name') ?? '';
    // Like
    card.querySelector('.home-card__action--like')?.addEventListener('click', async (e) => {
        e.stopPropagation();
    ...
    void fetch(`${BACKEND_URL}/feed/likes/${encodeURIComponent(itemId)}?userId=${encodeURIComponent(userId)}`, { cache: 'no-store' })
        .then(r => r.json())
        .then((d) => {
        const btn = card.querySelector('.home-card__action--like');
        if (btn)
            btn.classList.toggle('home-card__action--liked', d.liked);
        const el = card.querySelector(`[data-like-count="${itemId}"]`);
        if (el)
