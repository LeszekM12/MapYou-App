...
export async function syncJoinedClubsFromBackend() {
    try {
      ...
      <div id="sv2ClubResults" style="padding-bottom:32px"></div>`;
          ...
          ${clubs.map(c => {
              ...
              <div class="sv2-item" data-club-id="${c.clubId}" style="cursor:pointer">
                ...
                <div class="sv2-item__info">
                  ...
                  <span class="sv2-item__sub">${c.members.length} member${c.members.length !== 1 ? 's' : ''} · ${esc([c.city, c['region']].filter(Boolean).join(', '))}</span>
        ...
        <div class="sv2-list">
          ${mine.map(c => {
              ...
              <div class="sv2-item__info">
                <span class="sv2-item__name">${esc(c.name)}</span>
                <span class="sv2-item__sub">${c.members.length} members · ${esc([c.city, c.region].filter(Boolean).join(', '))}</span>
              </div>
            ...
            if (changed || pruned.length !== existingLocal.length)
                saveClubs(pruned);
    ...
    _openClubDetail(club) {
        ...
        <div class="sv2-club-detail__info">
          ...
          <div class="sv2-club-detail__meta">
            ...
            <span class="sv2-cd-meta__item">${club.isPrivate ? ICO.lock + 'Private' : ICO.globe + 'Public'}</span>
          </div>
        ...
        <div id="cdbMembersSection" style="${tab === 'members' ? '' : 'display:none'}">
          ...
          <div id="cdbPending"></div>` : ''}
            ...
            if (tab === 'members') {
                const host = modal.querySelector('#cdbMembers');
                if (host)
                    void renderMembersSection(host, club.id);
            }
            // Banner/logo upload
            ...
            modal.querySelector('#cdbJoin')?.addEventListener('click', async () => {
                ...
                if (club.joined) {
                    if (!confirm(`Leave "${esc(club.name)}"?`))
                        return;
            ...
            modal.querySelector('#cdbDelete')?.addEventListener('click', async () => {
                if (!confirm(`Delete "${esc(club.name)}"?`))
                    return;
            ...
            fetch(`${BACKEND_URL}/clubs/${encodeURIComponent(club.id)}/feed?requesterId=${encodeURIComponent(myUserId)}`, { cache: 'no-store' })
                ...
                <div style="display:flex;align-items:center;gap:10px;margin-top:6px">
                  <div data-profile-uid="${d.userId}" style="width:36px;height:36px;border-radius:50%;overflow:hidden;background:#444;flex-shrink:0;cursor:pointer">
                    ${d.avatarB64 ? `<img src="${safeUrl(d.avatarB64)}" style="width:100%;height:100%;object-fit:cover"/>` : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:#fff">${esc(d.authorName[0])}</div>`}
                  </div>
                ...
                import('./HomeView.js').then((hv) => {
                    const mod = hv;
                    const buildCard = mod.buildCard;
                    const buildPostCard = mod.buildPostCard;
                    data.data.forEach(f => {
                        const d = f.data;
                        ...
                        if (f.kind === 'post' && d.type === 'club_event') {
                            ...
                            ev.innerHTML = `<span style="color:rgba(255,255,255,0.4)">${esc(d.title)}</span><span style="color:rgba(255,255,255,0.25);margin-left:8px;font-size:1.1rem">${new Date(f.date).toLocaleDateString('en', { month: 'short', day: 'numeric' })}</span>`;
                        ...
                        if (canDelete) {
                            const existingMore = card.querySelector('.home-card__action--share');
                            ...
                            moreBtn.addEventListener('click', e => {
                                ...
                                delBtn.addEventListener('click', async () => {
                                    ...
                                    import('./cloudSync.js').then(cs => { const CS = cs.CS; CS?.deletePost?.(itemId); }).catch(() => { });
                                    loadFeed();
                                });
                        ...
                        if (f.kind === 'activity') {
                            ...
                            card.addEventListener('click', () => {
                                void import('./HomeView.js').then(async (hv) => {
                                    const fn = hv.openActivityDetail;
                                    if (fn)
                                        await fn({ ...d, id: itemId, coords: [] }, d.userId === myUserId, itemId);
                                });
                            });
                        }
                ...
                // 3-dot more menu
                feedEl.querySelectorAll('[data-more-post]').forEach(btn => {
                    btn.addEventListener('click', e => {
                        e.stopPropagation();
                        document.querySelectorAll('.sv2-more-menu').forEach(m => m.remove());
                ...
                feedEl.querySelectorAll('[data-profile-uid],[data-feed-profile]').forEach(el => {
                    el.addEventListener('click', () => {
                        ...
                        import('./PublicProfile.js').then(m => {
                            m.openPublicProfile(uid);
                            setTimeout(() => {
                                const pvEl = document.querySelector('.pv-overlay');
                                if (pvEl)
                                    pvEl.style.zIndex = '9500';
                                const w = setInterval(() => {
                                    if (!document.querySelector('.pv-overlay--visible')) {
                                        clearInterval(w);
                                        if (pvEl)
