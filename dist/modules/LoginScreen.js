...
import { signInWithGoogle, signInWithApple, exchangeSession, getDeviceLegacyState, getPlatform, } from './authService.js';
...
export function showLoginScreen() {
    return new Promise(resolve => {
      ...
      <div class="name-modal__card">
        ...
        const busy = (b) => {
            modal.querySelectorAll('button').forEach(x => (x.disabled = b));
        };
        ...
        const afterProviderSignIn = async () => {
            ...
            if (deviceUserId && !code) {
                showCodePanel('Na tym telefonie jest już konto MapYou. Wpisz jego kod odzyskiwania, ' +
        ...
        const providerFlow = async (provider) => {
        ...
        modal.querySelector('#loginTransfer')?.addEventListener('click', () => {
            showCodePanel('Wpisz kod odzyskiwania ze starego telefonu (Profil → Kod odzyskiwania). ' +
        ...
        function showCodePanel(message, allowNewAccount) {
        ...
        <p id="loginCodeErr" style="display:none;color:#ef4444;font-size:13px;margin:10px 0 0;"></p>`;
            const input = card.querySelector('#loginCodeInput');
            input.focus();
            const codeErr = card.querySelector('#loginCodeErr');
        ...
        function rebind() {
            ...
            modal.querySelector('#loginTransfer')?.addEventListener('click', () => {
                showCodePanel('Wpisz kod odzyskiwania ze starego telefonu (Profil → Kod odzyskiwania). ' +
                    'Po zalogowaniu Twoje konto zostanie przeniesione.', false);
            });
        }
    });
}
