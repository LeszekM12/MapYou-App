...
async function renderTemplate(ctx, tpl, act, statKeys) {
    ...
    if (tpl === 'transparent') {
        ...
        if ((act.coords?.length ?? 0) > 1) {
            ...
            _drawRouteFallback(ctx, act.coords, '#ffffff', rX, rY, rW, rH);
            _drawRouteFallback(ctx, act.coords, color, rX + 3, rY + 3, rW - 6, rH - 6);
            ctx.restore();
        }
...
export function openShareStudio(act) {
    ...
    if ((!act.coords || act.coords.length < 2)) {
        const enc = act.coordsEnc
            ?? act._coordsEncResolved;
        if (enc) {
            try {
                act = { ...act, coords: decodePolyline(enc) };
            }
            catch { /* leave empty */ }
        }
    }
    ...
    const canvases = templates.map((tpl) => {
    ...
    // is never dead.
    const dataUrlFromBlob = (blob) => new Promise(res => { const r = new FileReader(); r.onloadend = () => res(r.result); r.readAsDataURL(blob); });
    ...
    ov.querySelector('#ssDownload')?.addEventListener('click', async () => {
        ...
        if (r === 'saved')
            setTimeout(() => { lbl.textContent = 'Save image'; }, 1800);
    });
