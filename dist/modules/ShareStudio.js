// ─── SHARE STUDIO ────────────────────────────────────────────────────────────
// Style share sheet: swipe through several share-image templates, toggle
...
// ── shared drawing bits ──────────────────────────────────────────────────────
function drawStatsBlock(ctx, act, keys, x, y, w, color, onDark) {
...
function drawBrandMark(ctx, x, y, onDark) {
    ctx.textAlign = 'left';
...
async function drawRoute(ctx, act, x, y, w, h, color, tiles) {
    ...
    if (!drew)
        _drawRouteFallback(ctx, coords, color, x, y, w, h);
    ctx.restore();
}
// ── template renderers ───────────────────────────────────────────────────────
// Each fills the full canvas. `statKeys` chooses which numbers appear.
async function renderTemplate(ctx, tpl, act, statKeys) {
    const color = SPORT_COLORS[act.sport] ?? '#00c46a';
    const icon = SPORT_ICONS[act.sport] ?? '🏅';
    const title = act.name || act.description || getSportLabel(act.sport);
    ...
    if ((act.coords?.length ?? 0) > 1) {
        ...
        roundRect(ctx, 90, 340, CANVAS_W - 180, 900, 28);
        ctx.fill();
        ctx.textAlign = 'center';
        ctx.font = '120px sans-serif';
        ctx.fillText(icon, CANVAS_W / 2, 850);
        ctx.textAlign = 'left';
    }
...
// ── Studio UI ────────────────────────────────────────────────────────────────
export function openShareStudio(act) {
    document.getElementById('shareStudio')?.remove();
    const templates = [
        { id: 'map', name: 'Route' },
        ...(act.photoUrl ? [{ id: 'photo', name: 'Photo', needsPhoto: true }] : []),
    ...
    <div class="ss-sheet">
      ...
      <div class="ss-actions">
        <button class="ss-act" id="ssDownload">
          <span class="ss-act__ic">⬇️</span><span>Save image</span>
        </button>
        <button class="ss-act" id="ssShare">
          <span class="ss-act__ic">📤</span><span>Share</span>
        </button>
        <button class="ss-act" id="ssLink">
          <span class="ss-act__ic">🔗</span><span>Copy link</span>
        </button>
      </div>
    </div>`;
    ...
    const track = ov.querySelector('#ssTrack');
    const dots = ov.querySelector('#ssDots');
    // One preview canvas per template. Rendered lazily & re-rendered on toggle.
    const canvases = templates.map(() => {
        const slide = document.createElement('div');
        slide.className = 'ss-slide';
        const c = document.createElement('canvas');
        c.className = 'ss-canvas';
        c.width = CANVAS_W;
        c.height = CANVAS_H;
        slide.appendChild(c);
    ...
    const paint = async (i, force = false) => {
        if (rendered.has(i) && !force)
            return;
    ...
    const go = (i) => {
        ...
        void paint(current);
        void paint(current + 1);
        void paint(current - 1);
    };
    // Swipe.
    let startX = 0, dx = 0, dragging = false;
    const stage = ov.querySelector('#ssStage');
    ...
    // The flow: hand the IMAGE to the OS share sheet. From there the user
    // taps "Save Image" (→ Photos/Gallery, not Files) or sends it to Instagram,
    // Messages, etc. Web Share API Level 2 (files) works in the native WKWebView
    // and in Android WebView. `<a download>` is only a desktop-browser fallback,
    // because there it lands in Downloads, not a photo library.
    const shareImage = async (preferFile) => {
        const blob = await exportBlob();
        const link = activityDeepLink(act);
        const text = `${act.name || getSportLabel(act.sport)} — ${formatDistance(act.distanceKm)} via MapYou`;
        const nav = navigator;
        if (blob && preferFile) {
            const file = new File([blob], `mapyou-${act.sport}.png`, { type: 'image/png' });
            if (nav.canShare?.({ files: [file] })) {
                try {
                    await navigator.share({ files: [file], title: 'MapYou', text });
                    return;
                }
                catch (e) {
        ...
        // No file-share support (desktop) → download the PNG directly.
        if (blob) {
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `mapyou-${act.sport}-${act.id}.png`;
    ...
    ov.querySelector('#ssDownload')?.addEventListener('click', async () => {
        const lbl = ov.querySelector('#ssDownload span:last-child');
        lbl.textContent = 'Preparing…';
        await shareImage(true);
    ...
    ov.querySelector('#ssLink')?.addEventListener('click', async () => {
        const lbl = ov.querySelector('#ssLink span:last-child');
        try {
            await navigator.clipboard.writeText(activityDeepLink(act));
...
export function openShareActions(act) {
    document.getElementById('shareStudio')?.remove();
    const link = activityDeepLink(act);
    const ov = document.createElement('div');
    ov.id = 'shareStudio';
    ...
    <div class="ss-sheet ss-sheet--compact">
      ...
      <div class="ss-actions ss-actions--wide">
        <button class="ss-act" id="ssShare"><span class="ss-act__ic">📤</span><span>Share via…</span></button>
        <button class="ss-act" id="ssLink"><span class="ss-act__ic">🔗</span><span>Copy link</span></button>
      </div>
    </div>`;
    document.body.appendChild(ov);
    requestAnimationFrame(() => ov.classList.add('ss-overlay--visible'));
    ...
    ov.querySelector('#ssShare')?.addEventListener('click', async () => {
        try {
            if (navigator.share)
                await navigator.share({ title: 'MapYou', url: link });
        }
        catch { /* cancelled */ }
    });
    ov.querySelector('#ssLink')?.addEventListener('click', async () => {
...
//# sourceMappingURL=ShareStudio.js.map