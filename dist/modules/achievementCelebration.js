...
function target() {
    const el = document.querySelector('[data-tab="tabStats"]');
    if (el) {
...
async function showOne(a) {
    ...
    for (let i = 0; i < HOLD_MS / 100 && !skip; i++)
        await wait(100);
    // Odlot: sześciobok zostaje na ekranie jako osobny element, tlo gasnie.
    const gem = back.querySelector('#mapyouAchGem');
    if (gem) {
...
export async function celebrateNewAchievements() {
    if (busy)
        return;
...
//# sourceMappingURL=achievementCelebration.js.map