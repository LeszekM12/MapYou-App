...
export interface HealthProvider {
  ...
  /** Workouts recorded in [startMs, endMs) — newest first. */
  getWorkouts(startMs: number, endMs: number): Promise<HealthWorkout[]>;
}

/** One workout from Apple Health / Health Connect, normalised. */
export interface HealthWorkout {
  sourceId:    string;              // stable UUID from the health store (dedup key)
  sourceName:  string;              // recording app/device, e.g. "Samsung Health"
  sport:       string;              // mapped to MapYou sport keys
  startMs:     number;
  endMs:       number;
  durationSec: number;
  ...
  avgHr:       number | null;
  maxHr:       number | null;
  hrSeries:    Array<[number, number]>;  // [secOffset, bpm] downsampled (≤240 pts)
  laps:        Array<{ km: number; durationSec: number; paceMinKm: number }>; // per-km splits (when route has timestamps)
  coords:      Array<[number, number]>;  // GPS route ([] for indoor)
}
...
function mapHealthSport(t: string): string {
  ...
  if (k.includes('bik') || k.includes('cycl')) return 'cycling';
  if (k.includes('swim')) return 'swimming';
  if (k.includes('ski')) return 'skiing';
  if (k.includes('snowboard')) return 'snowboard';
  if (k.includes('row')) return 'rowing';
  if (k.includes('yoga')) return 'yoga';
  if (k.includes('strength') || k.includes('weight') || k.includes('gym')) return 'strength';
  return 'other';
}
...
class MockHealthProvider implements HealthProvider {
  ...
  async getWorkouts(startMs: number, endMs: number): Promise<HealthWorkout[]> {
    // One fake outdoor run yesterday + one indoor session — enough to exercise
    // the whole import UI in the browser.
    const y = startOfDay(Date.now() - DAY) + 8 * 3600_000;
    if (y < startMs || y > endMs) return [];
    const route: Array<[number, number]> = [];
    for (let i = 0; i <= 60; i++) {
      route.push([54.352 + Math.sin(i / 9.5) * 0.0035, 18.646 + (i / 60) * 0.012]);
    }
    const mkHr = (dur: number, base: number, amp: number): Array<[number, number]> => {
    ...
    return [
      {
        ...
        startMs: y, endMs: y + 31 * 60_000, durationSec: 31 * 60,
      ...
      },
      {
        ...
        startMs: y + 10 * 3600_000, endMs: y + 10 * 3600_000 + 45 * 60_000, durationSec: 45 * 60,
...
interface NativeHealthPlugin {
  ...
  queryWorkouts(opts: { startDate: string; endDate: string; includeHeartRate: boolean; includeRoute: boolean; includeSteps: boolean }): Promise<{ workouts?: NativeWorkout[] }>;
}
...
interface NativeWorkout {
  ...
  route?: Array<{ lat?: number; latitude?: number; lng?: number; longitude?: number; time?: string; timestamp?: number }>;
  heartRate?: Array<{ bpm?: number; value?: number; time?: string; startDate?: string; timestamp?: number }>;
}
...
class NativeHealthProvider implements HealthProvider {
  ...
  async getWorkouts(startMs: number, endMs: number): Promise<HealthWorkout[]> {
    ...
    try {
      ...
      return (r.workouts ?? []).map(w => this._normalise(w)).filter((w): w is HealthWorkout => w !== null)
        .sort((a, b) => b.startMs - a.startMs);
    } catch { return []; }
  }

  private _normalise(w: NativeWorkout): HealthWorkout | null {
    ...
    const pts = (w.route ?? [])
      ...
      .filter((c): c is { lat: number; lng: number; t: number } => c !== null);
    const coords: Array<[number, number]> = pts.map(p => [p.lat, p.lng]);
    // Per-km splits when route points carry timestamps
    const laps: Array<{ km: number; durationSec: number; paceMinKm: number }> = [];
...
export function getImportedHealthIds(): Set<string> {
  try { return new Set(JSON.parse(localStorage.getItem(IMPORTED_KEY) ?? '[]') as string[]); }
  catch { return new Set<string>(); }
}
export function markHealthImported(sourceId: string): void {
  const s = getImportedHealthIds(); s.add(sourceId);
  try { localStorage.setItem(IMPORTED_KEY, JSON.stringify([...s].slice(-500))); } catch { /* ignore */ }
}

/** Workouts from the last `days`, with already-imported ones flagged. */
export async function getImportableWorkouts(days = 14): Promise<Array<HealthWorkout & { imported: boolean }>> {
  const p = await getHealthProvider();
  const now = Date.now();
...
export async function openHealthConnectSettings(): Promise<void> {
  ...
  const p = cap.Plugins?.['HealthPlugin'] as { openHealthConnectSettings?: () => Promise<void>; openHealthSettings?: () => Promise<void> } | undefined;
  try {
    if (p?.openHealthConnectSettings) { await p.openHealthConnectSettings(); return; }
    if (p?.openHealthSettings) { await p.openHealthSettings(); return; }
  } catch { /* fall through */ }
  // Fallback: Android intent URI understood by the WebView shell
  try { window.open('intent://#Intent;action=android.health.connect.action.HEALTH_HOME_SETTINGS;end', '_system'); } catch { /* ignore */ }
}
