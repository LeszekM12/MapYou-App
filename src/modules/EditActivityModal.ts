...
export function openEditActivity(
  act: EditableActivity,
  userId: string,
  onSaved: (updated: Partial<EditableActivity>) => void,
): void {
  ...
  const nameEl  = $<HTMLInputElement>('eaName');
  const descEl  = $<HTMLTextAreaElement>('eaDesc');
  const notesEl = $<HTMLTextAreaElement>('eaNotes');
  const gallery = $<HTMLElement>('eaGallery');
  const fileEl  = $<HTMLInputElement>('eaFile');
  ...
  fileEl.addEventListener('change', () => {
    ...
    if (room <= 0) { alert(`Maximum ${MAX_PHOTOS} photos.`); return; }
  ...
  $('eaSave').addEventListener('click', () => {
    const btn = $<HTMLButtonElement>('eaSave');
    btn.disabled = true;
