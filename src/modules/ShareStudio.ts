...
type TemplateId = 'map' | 'photo' | 'minimal' | 'gradient';
interface Template { id: TemplateId; name: string; needsPhoto?: boolean }
...
export function openShareStudio(act: EnrichedActivity): void {
  ...
  const canvases: HTMLCanvasElement[] = templates.map(() => {
  ...
  // The Strava flow: hand the IMAGE to the OS share sheet. From there the user
  ...
  const shareImage = async (preferFile: boolean): Promise<void> => {
    ...
    if (blob && preferFile) {
      ...
      if (nav.canShare?.({ files: [file] })) {
        try { await navigator.share({ files: [file], title: 'MapYou', text }); return; }
        catch (e) { if ((e as Error).name === 'AbortError') return; }   // user cancelled — stop
      }
    }
