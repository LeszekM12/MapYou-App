...
import type { WeatherModal } from './WeatherModal.js';

const BAR_ID = 'weatherTopBar';

function buildBar(data: WeatherData): string {
...
export class WeatherTopBar {
  ...
  private _modal: WeatherModal;

  constructor(modal: WeatherModal) {
    this._modal = modal;
  }
   ...
   */
  mount(container: HTMLElement, data: WeatherData): void {
  ...
  update(data: WeatherData): void {
    if (!this._el) return;
    const loc  = this._el.querySelector<HTMLElement>('.wtb__location');
    const icon = this._el.querySelector<HTMLElement>('.wtb__wicon');
    const temp = this._el.querySelector<HTMLElement>('.wtb__temp');
    const desc = this._el.querySelector<HTMLElement>('.wtb__desc');
  ...
  private _bindEvents(): void {
    ...
    const open = () => {
      if (this._modal.isOpen) { this._modal.close(); }
      else                    { this._modal.open();  }
    };
