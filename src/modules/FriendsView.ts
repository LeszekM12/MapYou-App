...
export class FriendsView {
  ...
  private async _renderFeed(): Promise<void> {
    ...
    type Poz = { kind: string; date: number; data: Record<string, unknown> };

    const rysuj = async (poz: Poz[]): Promise<void> => {
      feedEl.innerHTML = '';
      koniecSzkieletu(feedEl);
      for (const item of poz) feedEl.appendChild(await this._buildFeedCard(item.kind, item.data));
    };
    ...
    // byly widoczne.
    const zCache = await cacheOdczytaj<Poz[]>(`friendsfeed:${userId}`);
    if (zCache?.value?.length) await rysuj(zCache.value);
    else pokazSzkieletFeedu(feedEl, 2);

    try {
      const res  = await fetch(`${BACKEND_URL}/feed?userId=${encodeURIComponent(userId)}`);
      if (!res.ok) { if (!zCache?.value?.length) feedEl.innerHTML = ''; return; }
      const data = await res.json() as { status: string; data: Poz[] };

      if (data.data.length) void cacheZapisz(`friendsfeed:${userId}`, data.data);

      if (!data.data.length) {
        if (zCache?.value?.length) return;   // mamy tresc — nie kasujemy jej
        koniecSzkieletu(feedEl);
