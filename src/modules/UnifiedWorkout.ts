...
export type WorkoutType = 'running' | 'walking' | 'cycling';
export type WorkoutSource = 'manual' | 'tracking';
...
function _fromManual(w: Record<string, any>): UnifiedWorkout {
...
function _fromEnriched(e: Record<string, any>): UnifiedWorkout {
...
function _fromActivity(a: Record<string, any>): UnifiedWorkout {
...
export function markWorkoutDeleted(id: string): void {
...
export async function migrateToUnified(): Promise<void> {
  // Always collect from all source tables and bulkPut (put = upsert, safe to re-run)
  const results: UnifiedWorkout[] = [];
  ...
  try {
    ...
    for (const w of manuals) {
      const u = _fromManual(w as Record<string, any>);
      if (!seenIds.has(u.id)) { seenIds.add(u.id); results.push(u); }
    }
  } catch {}
  ...
  try {
    ...
    for (const e of enriched) {
      const u = _fromEnriched(e as Record<string, any>);
      if (!seenIds.has(u.id)) { seenIds.add(u.id); results.push(u); }
    }
  } catch {}
  ...
  try {
    ...
    for (const a of activities) {
      const u = _fromActivity(a as Record<string, any>);
      if (!seenIds.has(u.id)) { seenIds.add(u.id); results.push(u); }
    }
  } catch {}
...
export function formatPaceSec(paceMinKm: number): string {
...
export const SPORT_ICONS_U: Record<WorkoutType, string> = {
