...
export async function getIPLocation(): Promise<IPLocationResult | null> {
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json() as {
        ...
        city?: string; country_name?: string; error?: boolean; reason?: string;
      };
      // ipapi.co returns error:true for localhost/reserved IPs
      if (!data.error && data.latitude && data.longitude) {
        const coords: Coords = [data.latitude, data.longitude];
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json() as {
        status?: string; lat?: number; lon?: number; city?: string; country?: string;
      };
      if (data.status === 'success' && data.lat && data.lon) {
        const coords: Coords = [data.lat, data.lon];
        _saveIPCache(coords, data.city ?? '', data.country ?? '');
        return { coords, city: data.city ?? '', country: data.country ?? '', source: 'ip' };
      }
    }
  ...
  try {
    ...
    if (cached) {
      const { coords, city, country } = JSON.parse(cached) as { coords: Coords; city: string; country: string };
  ...
  try {
    ...
    if (raw) {
      const coords = JSON.parse(raw) as Coords;
      return { coords, city: '', country: '', source: 'cache' };
    }
  } catch {}
...
function _saveIPCache(coords: Coords, city: string, country: string): void {
...
function _migrateOldCache(): void {
  if (localStorage.getItem(LS_IP_COORDS)) return; // already migrated
  try {
    const old = localStorage.getItem('mapty_last_coords');
    if (old) {
      const coords = JSON.parse(old) as Coords;
      localStorage.setItem(LS_IP_COORDS, JSON.stringify({ coords, city: '', country: '' }));
    }
  } catch {}
}
 ...
 */
export async function requestGPSPermission(): Promise<Coords | null> {
  try {
 ...
 */
export async function getBestLocation(): Promise<IPLocationResult | null> {
