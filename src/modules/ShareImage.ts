...
export async function _drawMapTiles(
  ...
  tmp.width  = cols * 256; tmp.height = rows * 256;
...
export async function generateShareImageFromEnriched(act: EnrichedActivity): Promise<void> {
  ...
  if (act.coords.length > 1) {
    ...
    if (transform) {
      ...
      ctx.strokeStyle = color; ctx.lineWidth = 6; ctx.shadowBlur = 0;
      ctx.beginPath();
      act.coords.forEach((c, i) => {
        const x = toCanvasX(c[1]), y = toCanvasY(c[0]);
        i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
...
function _tintToWhite(hex: string, amt: number): string {
  const h = hex.replace('#', '');
  const r = parseInt(h.slice(0, 2), 16), g = parseInt(h.slice(2, 4), 16), b = parseInt(h.slice(4, 6), 16);
  const mix = (c: number): number => Math.round(c + (255 - c) * (1 - amt));
  return `rgb(${mix(r)}, ${mix(g)}, ${mix(b)})`;
}

function _wrapText(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, maxW: number, lh: number, maxLines: number): number {
...
export async function composeActivityReel(
  act: EnrichedActivity,
  opts: { authorName?: string; avatarUrl?: string | null } = {},
): Promise<Blob | null> {
  const W = 1080, H = 1920;
  const canvas = document.createElement('canvas');
  ...
  const acx = PAD + 50;
  if (avatar) {
    ctx.save(); ctx.beginPath(); ctx.arc(acx, y, 50, 0, Math.PI * 2); ctx.clip();
    ctx.drawImage(avatar, acx - 50, y - 50, 100, 100); ctx.restore();
    ctx.lineWidth = 4; ctx.strokeStyle = color; ctx.beginPath(); ctx.arc(acx, y, 50, 0, Math.PI * 2); ctx.stroke();
  } else {
    ctx.fillStyle = color; ctx.beginPath(); ctx.arc(acx, y, 50, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.font = '600 46px system-ui'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText((opts.authorName?.[0] ?? '?').toUpperCase(), acx, y + 2);
  }
  ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
  ctx.fillStyle = '#16181c'; ctx.font = '600 42px system-ui';
  ctx.fillText(opts.authorName ?? 'Athlete', acx + 78, y - 6);
  ctx.fillStyle = '#8a8d93'; ctx.font = '400 30px system-ui';
  ...
  // Title
  y += 150;
  ctx.fillStyle = '#16181c'; ctx.font = '700 80px system-ui';
  const title = (act.name || '').replace(/^(undefined|null)\s*/i, '').trim() || getSportLabel(act.sport as SportType);
  y = _wrapText(ctx, title, PAD, y, W - PAD * 2, 90, 2);

  // Sport chip
  y += 46;
  ctx.font = '600 34px system-ui';
  const chipText = `${icon} ${getSportLabel(act.sport as SportType)}`;
  const chipW = ctx.measureText(chipText).width + 56;
  roundRect(ctx, PAD, y - 44, chipW, 64, 32); ctx.fillStyle = _tintToWhite(color, 0.25); ctx.fill();
  ctx.fillStyle = color; ctx.textBaseline = 'middle'; ctx.fillText(chipText, PAD + 28, y - 10); ctx.textBaseline = 'alphabetic';
  ...
  ctx.fillStyle = '#e8eef0'; ctx.fill();
  ctx.shadowColor = 'rgba(0,0,0,0.12)'; ctx.shadowBlur = 30; ctx.shadowOffsetY = 12; ctx.fill();
  ctx.restore();

  const coords = (Array.isArray(act.coords) ? act.coords : []) as [number, number][];
  if (coords.length > 1) {
    const transform = await _drawMapTiles(ctx, coords, mapX, mapY, mapW, mapH);
    ctx.save();
    roundRect(ctx, mapX, mapY, mapW, mapH, 40); ctx.clip();
    if (transform) {
      const { toCanvasX, toCanvasY } = transform;
      ctx.shadowColor = 'rgba(0,0,0,0.4)'; ctx.shadowBlur = 12;
      ctx.strokeStyle = 'rgba(255,255,255,0.95)'; ctx.lineWidth = 13; ctx.lineJoin = 'round'; ctx.lineCap = 'round';
      ctx.beginPath(); coords.forEach((c, i) => { const x = toCanvasX(c[1]), yy = toCanvasY(c[0]); i ? ctx.lineTo(x, yy) : ctx.moveTo(x, yy); }); ctx.stroke();
      ctx.shadowBlur = 0; ctx.strokeStyle = color; ctx.lineWidth = 7;
      ctx.beginPath(); coords.forEach((c, i) => { const x = toCanvasX(c[1]), yy = toCanvasY(c[0]); i ? ctx.lineTo(x, yy) : ctx.moveTo(x, yy); }); ctx.stroke();
      const dot = (lat: number, lng: number, fill: string): void => { const x = toCanvasX(lng), yy = toCanvasY(lat); ctx.beginPath(); ctx.arc(x, yy, 11, 0, Math.PI * 2); ctx.fillStyle = '#fff'; ctx.fill(); ctx.beginPath(); ctx.arc(x, yy, 7, 0, Math.PI * 2); ctx.fillStyle = fill; ctx.fill(); };
    ...
    ctx.save(); roundRect(ctx, mapX, mapY, mapW, mapH, 40); ctx.clip();
    const g = ctx.createLinearGradient(mapX, mapY, mapX, mapY + mapH);
    g.addColorStop(0, _tintToWhite(color, 0.35)); g.addColorStop(1, _tintToWhite(color, 0.18));
    ctx.fillStyle = g; ctx.fillRect(mapX, mapY, mapW, mapH);
    ctx.font = '200px system-ui'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(icon, mapX + mapW / 2, mapY + mapH / 2); ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
    ctx.restore();
  }
  ...
  stats.forEach((s, i) => {
    ...
    ctx.fillStyle = '#16181c'; ctx.font = '800 78px system-ui'; ctx.fillText(s.v, sx, y);
    ctx.fillStyle = '#8a8d93'; ctx.font = '500 30px system-ui'; ctx.fillText(s.l, sx, y + 50);
  });
  ctx.textAlign = 'left';

  // Footer — brand + tap hint
  ctx.fillStyle = color; ctx.font = '700 38px system-ui';
  ctx.fillText('MapYou', PAD, H - 90);
  ctx.fillStyle = '#8a8d93'; ctx.font = '500 30px system-ui'; ctx.textAlign = 'right';
