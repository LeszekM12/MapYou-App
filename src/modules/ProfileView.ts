 ...
 *  do konca tygodnia. */
export function recordWeeklyGoalWin(): boolean {
  const now     = new Date();
  const weekKey = `${now.getFullYear()}-W${_isoWeek(now)}`;
  if (localStorage.getItem(LS_GOAL_WEEK) === weekKey) return false;
...
export class ProfileView {
  ...
  async open(): Promise<void> {
    ...
    type ReelGroup = { userId: string; reels: { id: string }[]; hasUnseen: boolean };
    const reelData: ReelGroup[] = (reelRes?.ok ? ((await (reelRes as Response).json() as {data:ReelGroup[]}).data ?? []) : []);
    const myReel    = reelData.find(g => g.userId === myUserId);
    const hasReels  = !!myReel && myReel.reels.length > 0;
    const hasUnseen = hasReels && (myReel as ReelGroup).hasUnseen;
  ...
  private _buildShell(profile: ProfileData | ProfileRecord, hasReels = false, hasUnseen = false): HTMLElement {
  ...
  private _bindEvents(el: HTMLElement): void {
    ...
    el.querySelector('#pvAvatarReel')?.addEventListener('click', () => {
      const avatarEl = el.querySelector<HTMLElement>('#pvAvatarReel');
      if (!avatarEl?.classList.contains('pv-avatar--reel-active') && !avatarEl?.classList.contains('pv-avatar--reel-seen')) return;
      ...
      void (async () => {
        ...
        const d     = await res.json() as { data: { userId: string; reels: unknown[]; hasUnseen: boolean }[] };
        const group = d.data?.find(g => g.userId === myUid);
        if (group) openReelViewer(group, () => {
          if (avatarEl) { avatarEl.classList.remove('pv-avatar--reel-active'); avatarEl.classList.add('pv-avatar--reel-seen'); }
        });
      })();
    });
    ...
    el.querySelectorAll<HTMLElement>('.pv-act-item').forEach(item => {
      item.addEventListener('click', () => {
        ...
        void import('./HomeView.js').then(mod => {
          const fn = (mod as { openActivityDetail?: (a: unknown, isOwn: boolean, actId?: string) => void }).openActivityDetail;
          if (typeof fn === 'function') fn(act, true, w.id);
        });
      });
    });
  }
  ...
  private _statsWeekOffset  = 0;
  private _statsActiveSport = 'all';
  private _statsMetric: 'dist' | 'time' | 'count' = 'dist';
  private _statsChart: unknown = null;
  ...
  private _renderStatsWeek(el: HTMLElement): void {
    const now = new Date();
    ...
    if (labelEl) {
      if (this._statsWeekOffset === 0) { labelEl.textContent = 'This week'; }
      else {
        const fmt = (d: Date) => d.toLocaleDateString('en', { month: 'short', day: 'numeric' });
        labelEl.textContent = `${fmt(mon)} – ${fmt(sun)}`;
      }
    }
    (document.getElementById('pvStatsNext') as HTMLButtonElement|null)
      ?.[this._statsWeekOffset < 0 ? 'removeAttribute' : 'setAttribute']('disabled', '');
    ...
    const canvas  = document.getElementById('pvStatsChart') as HTMLCanvasElement | null;
    if (!canvas || typeof Chart === 'undefined') return;
    if (this._statsChart) (this._statsChart as Record<string,()=>void>).destroy?.();
    this._statsChart = new Chart(canvas, {
      ...
      options: {
        ...
        animation: { onComplete: function() {
          const chart = this as unknown as Record<string, unknown>;
          const ctx   = (chart.ctx as CanvasRenderingContext2D);
          const meta  = (chart.getDatasetMeta as (i: number) => {data: {x:number;y:number;base:number;width:number}[]})(0);
          ctx.save();
          ...
          meta.data.forEach((bar, i) => {
            const v = dayKm[i];
            if (v > 0) ctx.fillText(v.toFixed(1), bar.x, bar.y - 4);
          });
          ctx.restore();
        }},
      },
  ...
  private _bindStatsEvents(el: HTMLElement): void {
    ...
    document.getElementById('pvStatsPrev')?.addEventListener('click', () => {
      this._statsWeekOffset--; this._renderStatsWeek(el);
    });
    document.getElementById('pvStatsNext')?.addEventListener('click', () => {
      if (this._statsWeekOffset >= 0) return;
      this._statsWeekOffset++; this._renderStatsWeek(el);
    });
  }
  ...
  private async _renderServerAchievements(host: HTMLElement | null): Promise<void> {
    ...
    interface SrvAch {
      ...
      /** Obecne w `catalog` — pozwala pokazac takze odznaki DO ZDOBYCIA.
       *  Brak pola = stara odpowiedz serwera, traktujemy jako zdobyte. */
      earned?: boolean;
    }
    ...
    try {
      ...
      let d: { data?: SrvAch[]; catalog?: SrvAch[] } | null = null;
      try {
    ...
    try {
      ...
      if (rt.ok) {
        ...
        if (extra) host.insertAdjacentHTML('beforeend', extra);
      }
