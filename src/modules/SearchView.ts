...
export function getPendingClubs(): string[] {
  try { return JSON.parse(localStorage.getItem(LS_PENDING_CLUBS) ?? '[]'); } catch { return []; }
}
export function addPendingClub(clubId: string): void {
  const p = getPendingClubs(); if (!p.includes(clubId)) { p.push(clubId); localStorage.setItem(LS_PENDING_CLUBS, JSON.stringify(p)); }
}
export function removePendingClub(clubId: string): void {
    ...
    for (const cd of mine) {
      ...
      if (existingMap.has(cd.clubId)) {
        ...
        if (ex.joined !== joined || ex.isOwner !== isOwner) {
          ex.joined = joined; ex.isOwner = isOwner;
          changed = true;
        }
      } else {
        existing.push({
...
export class SearchView {
  ...
  open(tab: 'friends'|'clubs' = 'friends', openClubId?: string): void {
    ...
    requestAnimationFrame(() => {
      ...
      setTimeout(() => {
        el.querySelector<HTMLElement>('.sv2-sheet')?.classList.add('sv2-sheet--open');
  ...
  private _openClubById(clubId: string): void {
    fetch(`${BACKEND_URL}/clubs/${encodeURIComponent(clubId)}`, { cache: 'no-store' })
      .then(r => r.json())
      .then((d: { status: string; data: Record<string,unknown> }) => {
        if (d.status !== 'ok') return;
        const cd = d.data;
        const myUserId = getUserId();
        const localClub: LocalClub = {
          id:          cd.clubId as string,
          name:        cd.name as string,
          sport:       (cd.sport as string) ?? 'other',
          description: (cd.description as string) ?? '',
          location:    [cd.city, cd.region].filter(Boolean).join(', '),
          memberCount: (cd.members as string[]).length,
          isOwner:     cd.ownerId === myUserId,
          joined:      (cd.members as string[]).includes(myUserId),
          createdAt:   Date.now(), feed: [],
          logoB64:     (cd.avatarB64 as string) ?? undefined,
        };
        (localClub as unknown as Record<string,unknown>).isPrivate = cd.isPrivate ?? false;
  ...
  private _renderFriends(el: HTMLElement): void {
    ...
    let _szukObserver: IntersectionObserver | null = null;
    let _szukTrwa = false;

    const dolaczWyniki = (users: {userId:string;name:string;bio:string;avatarB64:string|null;city:string;region:string;followersCount:number}[]): void => {
    ...
    const podepnijDoladowywanie = (
      pobierz: (offset: number) => Promise<{ data: {userId:string;name:string;bio:string;avatarB64:string|null;city:string;region:string;followersCount:number}[]; hasMore: boolean } | null>,
      startOffset: number,
    ): void => {
      ...
      _szukObserver = new IntersectionObserver(async entries => {
        if (!entries[0]?.isIntersecting || _szukTrwa) return;
        _szukTrwa = true;
        ...
        try {
          ...
          if (!r || !r.hasMore) { _szukObserver?.disconnect(); sentinel.remove(); }
          else sentinel.innerHTML = '';
        } catch {
          sentinel.innerHTML = '';
        } finally { _szukTrwa = false; }
      }, { rootMargin: '300px' });
    ...
    const kartaUzytkownika = (u: {userId:string;name:string;bio:string;avatarB64:string|null;city:string;region:string;followersCount:number}): string => {
      {
        {
          ...
          return `
            <div class="sv2-item" data-userid="${u.userId}" style="cursor:pointer">
              ...
              <button class="sv2-badge ${isFollowing ? 'sv2-badge--gray' : 'sv2-badge--green'}"
                ...
                data-is-private="${(u as unknown as Record<string,unknown>).isPrivate ?? false}"
                data-pending="false"
              >${isFollowing ? 'Following' : ((u as unknown as Record<string,unknown>).isPrivate ? 'Request' : 'Follow')}</button>
            </div>`;
        }
      }
    };
    ...
    const searchUsers = async (q: string) => {
      ...
      try {
        const url = (off: number): string =>
          `${BACKEND_URL}/users/search?q=${encodeURIComponent(q)}`
          + `&exclude=${encodeURIComponent(myUserId)}&offset=${off}`;
        const res  = await fetch(url(0));
        const data = await res.json() as { status: string; hasMore?: boolean; data: {userId:string;name:string;bio:string;avatarB64:string|null;city:string;region:string;followersCount:number}[] };
        renderUserList(data.data ?? [], `Results for "${q}"`);
        if (data.hasMore) {
          podepnijDoladowywanie(async off => {
            const r = await fetch(url(off));
            const d = await r.json() as { hasMore?: boolean; data?: typeof data.data };
  ...
  private _renderClubs(el: HTMLElement): void {
      ...
      <div class="sv2-search-wrap">
        ...
        <div class="sv2-location-pill" id="sv2LocationBtn" style="cursor:pointer;user-select:none">
          ${(() => {
            ...
            if (ov !== null) return ov ? `📍 Near ${ov} ▾` : '🌍 All locations ▾';
    ...
    const renderClubs = (clubs: {clubId:string;name:string;sport:string;city:string;region?:string;description:string;members:string[];avatarB64:string|null}[], title: string) => {
      ...
      resultsEl.querySelectorAll<HTMLElement>('.sv2-item[data-club-id]').forEach(item => {
        item.addEventListener('click', e => {
          if ((e.target as HTMLElement).closest('[data-club-join]')) return;
      ...
      resultsEl.querySelectorAll<HTMLButtonElement>('[data-club-join]').forEach(btn => {
        btn.addEventListener('click', async e => {
          ...
          if (subEl) {
            ...
            if (m) { const n = joined ? Math.max(0, Number(m[1]) - 1) : Number(m[1]) + 1; subEl.textContent = subEl.textContent!.replace(m[0], n + ' member' + (n !== 1 ? 's' : '')); }
          }
    ...
    const renderMyClubs = (backendClubs: {clubId:string;name:string;sport:string;city:string;region?:string;description:string;members:string[];avatarB64:string|null;ownerId:string;isPrivate?:boolean}[]) => {
      const sportIcons: Record<string,string> = { running:'🏃',walking:'🚶',cycling:'🚴',fitness:'💪',hiking:'🥾',other:'🏅' };
      ...
      if (!mine.length) { mySection.innerHTML = ''; return; }
      mySection.innerHTML = `
  ...
  private _openClubDetail(club: LocalClub): void {
    ...
    const fetchClub = async (tab: 'feed'|'members'|'events' = 'feed'): Promise<void> => {
      try {
        ...
        startFeedPolling();
        const fresh = d.data;
        if (fresh.isPrivate !== undefined) (club as unknown as Record<string,unknown>).isPrivate = fresh.isPrivate;
        if (fresh.members)    club.memberCount = (fresh.members as string[]).length;
        if (fresh.avatarB64)  club.logoB64     = fresh.avatarB64 as string;
        if (fresh.bannerUrl)  club.bannerB64   = fresh.bannerUrl as string;
        if (fresh.members) club.joined = (fresh.members as string[]).includes(myUserId);
        ...
        if (c) {
          if (club.bannerB64) c.bannerB64 = club.bannerB64;
          if (club.logoB64)   c.logoB64   = club.logoB64;
          c.memberCount = club.memberCount; c.joined = club.joined;
          saveClubs(clubs);
        }
        renderModal(tab);
    ...
    const renderModal = (tab: 'feed'|'members'|'events' = 'feed') => {
      ...
      modal.querySelector('#cdbJoin')?.addEventListener('click', async () => {
        ...
        if (club.joined) {
          ...
          const lcs2 = loadClubs(); const lc2 = lcs2.find(c => c.id === club.id);
          if (lc2) { lc2.joined = false; lc2.memberCount = club.memberCount; saveClubs(lcs2); }
        } else if (isPrivate) {
      ...
      modal.querySelector('#cdbAddPost')?.addEventListener('click', () => {
        import('./PostModal.js').then(m => {
          m.openPostModal(async post => {
            ...
            const feedEl = modal.querySelector<HTMLElement>('#cdbFeed');
            if (feedEl) {
    ...
    const startFeedPolling = () => {
      ...
      _feedTimer = setInterval(() => {
        if (modal.isConnected && modal.querySelector('#cdbFeedSection') && (modal.querySelector('#cdbFeedSection') as HTMLElement).style.display !== 'none') loadFeed();
      }, 30000);
    };
    const stopFeedPolling = () => { if (_feedTimer) { clearInterval(_feedTimer); _feedTimer = null; } };
    ...
    const loadFeed = () => {
      ...
      fetch(`${BACKEND_URL}/clubs/${encodeURIComponent(club.id)}/feed?requesterId=${encodeURIComponent(myUserId)}`, { cache:'no-store' })
        ...
        .then((data: { status: string; data: {kind:string;date:number;data:Record<string,unknown>}[] }) => {
          ...
          // Like handlers
          feedEl.querySelectorAll<HTMLElement>('[data-club-like]').forEach(btn => {
            const itemId   = btn.dataset.clubLike!;
            const itemType = btn.dataset.itemType as 'post'|'activity';
            // Fetch initial state
            void fetch(`${BACKEND_URL}/feed/likes/${encodeURIComponent(itemId)}?userId=${encodeURIComponent(myUserId)}`, { cache:'no-store' })
              ...
              .then((d: { liked:boolean; count:number }) => {
                ...
                const el = feedEl.querySelector<HTMLElement>(`[data-like-count="${itemId}"]`);
          ...
          // Comment handlers
          feedEl.querySelectorAll<HTMLElement>('[data-club-comment]').forEach(btn => {
            const itemId   = btn.dataset.clubComment!;
            btn.addEventListener('click', e => {
              e.stopPropagation();
              const card = btn.closest<HTMLElement>('[data-feed-post-id]')!;
    ...
    const spinner = ptr;
    const spinDot = ptr.querySelector<HTMLElement>('.home-ptr__spinner')!;

    let ptrStartY = 0, ptrPull = 0, ptrActive = false, ptrBusy = false;
    const PTR_TRIGGER = 70;

    modal.addEventListener('touchstart', ev => {
      if (ptrBusy || modal.scrollTop > 0) { ptrActive = false; return; }
      ptrActive = true; ptrStartY = ev.touches[0].clientY; ptrPull = 0;
    }, { passive: true });

    modal.addEventListener('touchmove', ev => {
      if (!ptrActive || ptrBusy) return;
      ptrPull = (ev.touches[0].clientY - ptrStartY) * 0.45;   // opór
      if (ptrPull <= 0) { spinner.style.transform = ''; spinner.style.opacity = '0'; spinDot.style.transform = ''; return; }
      if (modal.scrollTop > 0) { ptrActive = false; return; }
    ...
    modal.addEventListener('touchend', () => {
      ...
      if (ptrPull < PTR_TRIGGER) {                       // za mało — cofnij
        spinner.style.transform = ''; spinner.style.opacity = '0'; spinDot.style.transform = '';
        return;
      }
      ptrBusy = true;
    ...
    setTimeout(() => { if (!modal.querySelector('.sv2-club-detail__action-row')) renderModal('feed'); }, 1000);
  }
