...
interface PostFull {
  ...
  photoUrl: string | null; date: number;
  authorName: string; avatarB64: string | null;
  visibility?: Visibility;
}

interface CommentRow {
  commentId: string; itemId: string; userId: string;
  authorName: string; text: string; avatarB64?: string | null;
  createdAt: string;
}
...
const VIS_META: Record<Visibility, { ic: string; t: string }> = {
...
async function fetchPost(postId: string): Promise<PostFull | null> {
  try {
    const r = await fetch(`${BACKEND_URL}/posts/single/${encodeURIComponent(postId)}`, { cache: 'no-store' });
    const j = await r.json() as { status: string; data: PostFull };
...
async function fetchComments(postId: string): Promise<CommentRow[]> {
  try {
    ...
    const j = await r.json() as { status: string; data: CommentRow[] };
...
async function deleteComment(commentId: string): Promise<boolean> {
  try {
    const r = await fetch(
      `${BACKEND_URL}/feed/comment/${encodeURIComponent(commentId)}?userId=${encodeURIComponent(getUserId())}`,
      { method: 'DELETE' });
    return r.ok;
  } catch { return false; }
}
...
async function deletePost(postId: string): Promise<boolean> {
  try {
    const r = await fetch(
      `${BACKEND_URL}/posts/${encodeURIComponent(postId)}?userId=${encodeURIComponent(getUserId())}`,
      { method: 'DELETE' });
    return r.ok;
  } catch { return false; }
}

async function patchVisibility(postId: string, visibility: Visibility): Promise<boolean> {
  try {
...
function openCommentOptions(c: CommentRow, onDeleted: () => void): void {
  ...
  sheet.querySelector('#pcReply')?.addEventListener('click', () => {
    close();
    const input = document.getElementById('pdInput') as HTMLInputElement | null;
    if (input) { input.value = `@${c.authorName} `; input.focus(); }
  });
  ...
  sheet.querySelector('#pcDelete')?.addEventListener('click', async () => {
    close();
    if (await deleteComment(c.commentId)) onDeleted();
  });
}

// ── Post options (author): visibility + delete ───────────────────────────────

function openPostOptions(post: PostFull, badgeEl: HTMLElement | null, onDeleted: () => void): void {
  const cur: Visibility = post.visibility ?? 'everyone';
  const opts: Visibility[] = ['everyone', 'friends', 'only_me'];
  const sheet = document.createElement('div');
  sheet.className = 'adm-overlay';
  sheet.innerHTML = `
    <div class="adm-sheet">
  ...
  sheet.querySelectorAll<HTMLElement>('[data-vis]').forEach(b => {
    b.addEventListener('click', async () => {
      const v = b.dataset.vis as Visibility;
      post.visibility = v;
      if (badgeEl) badgeEl.textContent = `${VIS_META[v].ic} ${VIS_META[v].t}`;
      close();
  ...
  sheet.querySelector('#pdDelete')?.addEventListener('click', async () => {
    close();
    if (await deletePost(post.postId)) onDeleted();
  });
}

// ── Main entry ───────────────────────────────────────────────────────────────

export async function openPostDetail(postId: string, onChange?: () => void): Promise<void> {
  ...
  const sheet = ov.querySelector<HTMLElement>('.pd-sheet')!;
  if (!post) { sheet.innerHTML = `<div class="pd-empty"><span>📡</span><p>Post not found</p><button class="ev-btn ev-btn--ghost" id="pdClose">Close</button></div>`;
    sheet.querySelector('#pdClose')?.addEventListener('click', close); return; }

  const isAuthor = post.userId === getUserId();
  const vis: Visibility = post.visibility ?? 'everyone';

  const paintComments = (list: CommentRow[]) => `
  ...
  const render = (comments: CommentRow[]) => {
    sheet.innerHTML = `
      <div class="pd-head">
    ...
    sheet.querySelector('#pdOpts')?.addEventListener('click', () =>
      openPostOptions(post, sheet.querySelector<HTMLElement>('#pdVisBadge'), () => { close(); onChange?.(); }));

    // Media which 404s → hide instead of a broken frame (same pattern as feeds).
    sheet.querySelector<HTMLImageElement>('.pd-post__photo')?.addEventListener('error', e => {
      (e.target as HTMLElement).style.display = 'none';
    });

    sheet.querySelectorAll<HTMLElement>('.pd-comment').forEach(el => {
      const c = comments.find(x => x.commentId === el.dataset.cid);
      if (c) el.addEventListener('click', () => openCommentOptions(c, () => void reload()));
    });

    const input = sheet.querySelector<HTMLInputElement>('#pdInput')!;
    const send  = async () => {
      const text = input.value.trim();
      if (!text) return;
      input.value = '';
      if (await addComment(postId, text)) { await reload(); onChange?.(); }
    };
