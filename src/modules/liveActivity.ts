...
function registryAdd(id: string): void {
  try {
...
function registryRemove(id: string): void {
  try {
    localStorage.setItem(REGISTRY_KEY,
      JSON.stringify(registryRead().filter(x => x !== id)));
  } catch { /* nothing sensible to do */ }
}

function registryClear(): void {
...
class WorkoutLiveActivity {
   ...
   */
  private async _dismissOne(p: LAPlugin, id: string, data: Record<string, unknown>): Promise<boolean> {
    // Warm-up: forces the plugin to re-sync. Its result is irrelevant.
    try { await p.updateActivity({ activityId: id, data }); }
   ...
   */
  private async _sweepOrphans(): Promise<void> {
    const p = laPlugin();
    const ids = registryRead();
    ...
    for (const id of ids) {
      if (await this._dismissOne(p, id, {})) registryRemove(id);
    }
  }
  ...
  async start(sportKey: string, sportLabel: string, paused = false): Promise<void> {
    ...
    try {
      ...
      if (this._id) registryAdd(this._id);
    } catch (e) {
  ...
  async end(final?: LiveStats): Promise<void> {
    ...
    this._lastPush = 0;
    this._paused = false;
    if (!p || !ids.length) { registryClear(); return; }
    ...
    for (const id of ids) {
      if (await this._dismissOne(p, id, finalData)) registryRemove(id);
      else stubborn++;
    }

    if (!stubborn) registryClear();
    else console.warn(`[LiveActivity] ${stubborn} card(s) left; will retry on next start`);
  }

  /** What the registry currently holds. Diagnostics only. */
  get trackedIds(): string[] { return registryRead(); }
}
 ...
 */
export function sweepOrphanActivities(): void {
  void (workoutLiveActivity as unknown as {
...
if (typeof document !== 'undefined') {
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') sweepOrphanActivities();
  });
}

/** What the registry holds right now. Diagnostics only. */
(globalThis as unknown as Record<string, unknown>).mapyouLA = (): unknown => ({
