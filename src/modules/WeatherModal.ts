...
function buildModal(data: WeatherData): string {
  ...
  const hourlyHTML = hourly.map(h => {
    ...
    if ((h as {isSunrise?: boolean}).isSunrise) return `
...
export class WeatherModal {
  ...
  open(): void {
    ...
    requestAnimationFrame(() => {
      const sheet = this._el!.querySelector<HTMLElement>('.wm-sheet');
      if (sheet) sheet.classList.add('wm-sheet--open');
    });
  }
  ...
  close(): void {
    ...
    setTimeout(() => {
      if (this._el) this._el.classList.remove('wm-overlay--visible');
  ...
  private _bindEvents(): void {
    const el    = this._el!;
    const sheet = el.querySelector<HTMLElement>('.wm-sheet')!;
    const handle= el.querySelector<HTMLElement>(`#${MODAL_ID}Handle`)!;
    ...
    handle.addEventListener('mousedown', e => {
      ...
      const onMove = (ev: MouseEvent) => moveSwipe(ev.clientY);
      const onUp   = (ev: MouseEvent) => { endSwipe(ev.clientY); document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); };
