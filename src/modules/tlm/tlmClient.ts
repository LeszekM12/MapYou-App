...
export function deriveTlmId(publicKey: Uint8Array): string {
  const hash = sodium.crypto_hash_sha256(publicKey);
  const raw = base32(hash, 8);
  return raw.slice(0, 4) + '-' + raw.slice(4);
}

export interface TlmIdentity {
  tlmId: string;
...
export async function ensureTlmIdentity(): Promise<TlmIdentity> {
  await sodium.ready;
...
export function encryptFor(plaintext: string, theirPk: Uint8Array, mySk: Uint8Array): string {
...
export function decryptFrom(payloadB64: string, theirPk: Uint8Array, mySk: Uint8Array): string {
...
export async function resolveMapyouUser(userId: string): Promise<{ tlmId: string; publicKey: string } | null> {
...
export class TlmGatewaySocket {
  ...
  private identity: TlmIdentity;
  private listeners: FrameCb[] = [];
  private queue: any[] = [];
  private ready = false;
  private closedByUs = false;

  constructor(identity: TlmIdentity) {
  ...
  connect() {
    ...
    this.ws.onclose = () => {
      this.ready = false;
      if (!this.closedByUs) setTimeout(() => this.connect(), 3000); // auto-reconnect
    };
    this.ws.onerror = () => { /* onclose zrobi reconnect */ };
  }

  /** Wysyla ramke; przed 'ready' trafia do kolejki. */
  send(frame: any) {
    if (this.ready) this.rawSend(frame);
    else this.queue.push(frame);
  }

  private rawSend(frame: any) {
