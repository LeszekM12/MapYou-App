...
interface CsvActivity {
  stravaId: string;
  dateStr:  string;   // localised — used only as fallback when no file
  name:     string;
  typeStr:  string;   // localised sport name
  desc:     string;
  elapsedSec: number;
  movingSec:  number | null;  // heuristic column — validated before use
  filename: string | null;    // e.g. "activities/123456.gpx.gz"
}

interface ParsedTrack {
  points: Array<{ lat: number; lng: number; t: number | null; ele: number | null; hr: number | null; distM: number | null }>;
  sportHint: string | null;   // from TCX Sport attr / FIT session
  totalDistM: number | null;  // from FIT session / TCX DistanceMeters
  totalTimerSec: number | null; // FIT moving time
  startMs: number | null;
}
...
function unifiedType(sport: string): 'running' | 'walking' | 'cycling' {
  if (CYCLE_SPORTS.has(sport)) return 'cycling';
  if (sport === 'walking' || sport === 'hiking' || sport === 'snowshoe') return 'walking';
  return 'running';
}
...
function parseActivitiesCsv(text: string): CsvActivity[] {
  ...
  return body
    ...
    .map(r => {
      ...
      if (movingCol >= 0 && r[movingCol] != null) {
        ...
        if (Number.isFinite(m) && m > 0 && m <= elapsed * 1.05) moving = m;
      }
      return {
        stravaId: r[0].trim(),
        dateStr:  r[1]?.trim() ?? '',
...
function parseGpx(xml: string): ParsedTrack {
  ...
  for (let i = 0; i < trkpts.length; i++) {
    ...
    for (let j = 0; j < all.length; j++) {
      if (all[j].localName === 'hr') { hr = Number(all[j].textContent) || null; break; }
    }
    pts.push({
      lat, lng,
...
function parseTcx(xml: string): ParsedTrack {
  ...
  for (let i = 0; i < tps.length; i++) {
    ...
    if (dEl?.textContent) lastDist = Number(dEl.textContent) || lastDist;
...
function parseFit(buf: Uint8Array): ParsedTrack {
  ...
  interface MsgDef { global: number; little: boolean; fields: FieldDef[]; devBytes: number }
  const defs = new Map<number, MsgDef>();

  const pts: ParsedTrack['points'] = [];
  let sportHint: string | null = null;
  let totalDistM: number | null = null;
  let totalTimerSec: number | null = null;
  let lastTs: number | null = null; // for compressed-timestamp headers

  const SC = 180 / 2 ** 31; // semicircles → degrees

  const readField = (little: boolean, size: number, base: number): number | null => {
    const t = base & 0x1f;
    let v: number;
    switch (t) {
      case 0x00: case 0x02: v = dv.getUint8(off); if (v === 0xff) return null; break;         // enum/uint8
      case 0x01: v = dv.getInt8(off);  if (v === 0x7f) return null; break;                    // sint8
      case 0x03: v = dv.getInt16(off, little); if (v === 0x7fff) return null; break;          // sint16
      case 0x04: v = dv.getUint16(off, little); if (v === 0xffff) return null; break;         // uint16
      case 0x05: v = dv.getInt32(off, little); if (v === 0x7fffffff) return null; break;      // sint32
      case 0x06: case 0x0c: v = dv.getUint32(off, little); if (v === 0xffffffff) return null; break; // uint32
  ...
  while (off < end) {
    ...
    if (hdr & 0x80) {
      ...
      if (lastTs != null) lastTs = (lastTs & ~0x1f) + tsOffset + (tsOffset < (lastTs & 0x1f) ? 0x20 : 0);
      readDataMsg(def, lastTs);
    } else if (hdr & 0x40) {
      // Definition message
      const local = hdr & 0x0f;
      off++; // reserved
      const little = dv.getUint8(off) === 0; off++;
      const global = little ? dv.getUint16(off, true) : dv.getUint16(off, false); off += 2;
      ...
      if (hdr & 0x20) { // developer fields present
        const dn = dv.getUint8(off); off++;
        for (let i = 0; i < dn; i++) { devBytes += dv.getUint8(off + 1); off += 3; }
      }
  ...
  function readDataMsg(def: MsgDef, compressedTs: number | null): void {
    let lat: number | null = null, lng: number | null = null, ele: number | null = null;
    let hr: number | null = null, dist: number | null = null, ts: number | null = compressedTs;
    for (const f of def.fields) {
      ...
      if (def.global === 20) {          // record
        if (f.num === 253 && v != null) ts = v;
        else if (f.num === 0 && v != null) lat = v * SC;
        else if (f.num === 1 && v != null) lng = v * SC;
        else if (f.num === 2 && v != null) ele = v / 5 - 500;
        else if (f.num === 78 && v != null) ele = v / 5 - 500;
        else if (f.num === 3 && v != null) hr = v;
        else if (f.num === 5 && v != null) dist = v / 100;
      } else if (def.global === 18) {   // session
        if (f.num === 5 && v != null) sportHint = FIT_SPORT[v] ?? sportHint;
        else if (f.num === 8 && v != null) totalTimerSec = v / 1000;
        else if (f.num === 9 && v != null) totalDistM = v / 100;
      }
      off += f.size;
    }
    off += def.devBytes;
...
function trackMetrics(t: ParsedTrack): {
  distanceKm: number; elevGain: number;
  hrSeries: Array<[number, number]> | null; avgHr: number | null; maxHr: number | null;
  elevSeries: Array<[number, number]> | null;
} {
  const pts = t.points;
  ...
  if (!distM && pts.length > 1) {
    for (let i = 1; i < pts.length; i++) {
      ...
      if (d < 100) distM += d; // GPS-jump guard
    }
  }
  // Elevation gain: positive deltas over a small noise floor
  let elevGain = 0; let lastEle: number | null = null;
  const elevSeries: Array<[number, number]> = [];
  let cumM = 0;
  for (let i = 0; i < pts.length; i++) {
    if (i > 0) cumM += Math.min(haversineM(pts[i - 1], pts[i]), 100);
    const e = pts[i].ele;
    if (e == null) continue;
    if (lastEle != null && e - lastEle > 0.5) elevGain += e - lastEle;
    lastEle = e;
  ...
  let hrSeries: Array<[number, number]> | null = null;
  let avgHr: number | null = null, maxHr: number | null = null;
  if (hrRaw.length > 3 && start != null) {
    ...
    for (const p of hrRaw) {
      ...
      sum += p.hr!; if (p.hr! > maxHr) maxHr = p.hr!;
      if (sec >= nextAt) { hrSeries.push([sec, p.hr!]); nextAt = sec + step; }
    }
...
function parseCsvDate(s: string): number | null {
  ...
  if (m && PL_MONTHS[m[2]] != null) {
    return new Date(Number(m[3]), PL_MONTHS[m[2]], Number(m[1]),
...
declare const JSZip: {
  loadAsync(data: ArrayBuffer | Uint8Array): Promise<{
    file(path: string): { async(type: 'string'): Promise<string>; async(type: 'uint8array'): Promise<Uint8Array> } | null;
    files: Record<string, unknown>;
  }>;
};

async function parseActivityFile(zip: Awaited<ReturnType<typeof JSZip.loadAsync>>, path: string): Promise<ParsedTrack | null> {
  ...
  if (lower.endsWith('.gpx')) return parseGpx(new TextDecoder().decode(bytes));
  if (lower.endsWith('.tcx')) return parseTcx(new TextDecoder().decode(bytes));
...
export async function importStravaArchive(
  ...
  if (!csvEntry) throw new Error('W archiwum nie znaleziono activities.csv — czy to na pewno eksport ze Stravy?');
  ...
  const summary: ImportSummary = { total: activities.length, imported: 0, duplicates: 0, failed: 0, failedNames: [] };
  let done = 0;
  ...
  for (const a of activities) {
    ...
    if (existing.has(id)) { summary.duplicates++; continue; }

    try {
      let track: ParsedTrack | null = null;
      if (a.filename) track = await parseActivityFile(zip, a.filename);
      ...
      const coords: Array<[number, number]> = (track?.points ?? []).map(p => [p.lat, p.lng]);
      ...
      summary.failed++;
      if (summary.failedNames.length < 10) summary.failedNames.push(a.name || a.stravaId);
...
export function showStravaImportModal(): void {
  ...
  const input  = modal.querySelector<HTMLInputElement>('#stravaZipInput')!;
  const status = modal.querySelector<HTMLElement>('#stravaImportStatus')!;
  const bar    = modal.querySelector<HTMLElement>('#stravaImportBar')!;
  const fill   = modal.querySelector<HTMLElement>('#stravaImportBarFill')!;
  const startBtn = modal.querySelector<HTMLButtonElement>('#stravaImportStart')!;
  const closeBtn = modal.querySelector<HTMLButtonElement>('#stravaImportClose')!;
    ...
    try {
      ...
      void (async () => {
        try {
          ...
          const d = await r.json() as { earned?: number };
          if (d.earned) {
      ...
      startBtn.disabled = false; input.disabled = false;
    }
  });
}
