...
interface PollOption { id: string; label: string; count: number | null; voters?: string[] }

interface Poll {
  pollId: string; question: string; type: PollType;
  creatorId: string; creatorName: string;
  anonymous: boolean; hideResults: boolean;
  endAt: number | null; closed: boolean;
  myVotes: string[]; voted: boolean; reveal: boolean;
...
export async function fetchPolls(clubId: string): Promise<Poll[]> {
  try {
    const r = await fetch(
      `${BACKEND_URL}/clubs/${encodeURIComponent(clubId)}/polls?userId=${encodeURIComponent(getUserId())}`,
      { cache: 'no-store' });
    const j = await r.json() as { status: string; data: Poll[] };
    return j.status === 'ok' ? j.data : [];
  } catch { return []; }
}

async function vote(pollId: string, optionIds: string[]): Promise<Poll | null> {
  try {
...
function pollCardHtml(p: Poll): string {
  const me    = getUserId();
  const total = p.totalVotes ?? 0;
  const multi = p.type === 'multiple';

  const opt = (o: PollOption) => {
...
function bindPollCard(el: HTMLElement, p: Poll, onChange: () => void): void {
  el.querySelectorAll<HTMLElement>('[data-opt]').forEach(b => {
    b.addEventListener('click', async () => {
      ...
      if (fresh) { el.outerHTML = pollCardHtml(fresh); onChange(); }
    });
  });
  el.querySelector('[data-del]')?.addEventListener('click', async () => {
    if (!confirm('Delete this poll?\n\nVotes will be lost.')) return;
    if (await deletePoll(p.pollId)) onChange();
  });
}

/** Render polls into a host element. Re-binds after every vote. */
export async function renderPolls(host: HTMLElement, clubId: string): Promise<void> {
  const polls = await fetchPolls(clubId);
  if (!polls.length) { host.innerHTML = ''; return; }
  const paint = () => {
    host.innerHTML = polls.map(pollCardHtml).join('');
    host.querySelectorAll<HTMLElement>('.pl-card').forEach(el => {
      const p = polls.find(x => x.pollId === el.dataset.poll);
      if (p) bindPollCard(el, p, () => void renderPolls(host, clubId));
    });
  };
  paint();
}
...
export function openCreatePollModal(clubId: string, onCreated: () => void): void {
  ...
  m.innerHTML = `
    <div class="ev-modal__card">
      ...
      <div class="pl-types" id="plTypes">
        ${(Object.keys(TYPE_META) as PollType[]).map(t => `
  ...
  const optsWrap = m.querySelector<HTMLElement>('#plOptsWrap')!;
  const optsBox  = m.querySelector<HTMLElement>('#plOpts')!;

  m.querySelectorAll<HTMLElement>('[data-type]').forEach(b => {
    b.addEventListener('click', () => {
      type = b.dataset.type as PollType;
  ...
  m.querySelector('#plAddOpt')?.addEventListener('click', () => {
    if (optsBox.children.length >= 6) return;   // 6 = server-side cap
    const i = optsBox.children.length;
    const inp = document.createElement('input');
    inp.className = 'pl-opt-in'; inp.type = 'text'; inp.maxLength = 60;
  ...
  m.querySelector('#plCreate')?.addEventListener('click', async () => {
    const err = m.querySelector<HTMLElement>('#plErr')!;
    const question = m.querySelector<HTMLInputElement>('#plQ')!.value.trim();
    if (!question) { err.textContent = 'Write the question first.'; return; }

    const options = type === 'yesno' ? [] :
      [...optsBox.querySelectorAll<HTMLInputElement>('.pl-opt-in')].map(i => i.value.trim()).filter(Boolean);
    if (type !== 'yesno' && options.length < 2) { err.textContent = 'Give at least 2 options.'; return; }

    const endStr = m.querySelector<HTMLInputElement>('#plEnd')!.value;
    const btn = m.querySelector<HTMLButtonElement>('#plCreate')!;
    ...
    try {
      const r = await fetch(`${BACKEND_URL}/clubs/${encodeURIComponent(clubId)}/polls`, {
        ...
        body: JSON.stringify({
          ...
          anonymous:   m.querySelector<HTMLInputElement>('#plAnon')!.checked,
          hideResults: m.querySelector<HTMLInputElement>('#plHide')!.checked,
