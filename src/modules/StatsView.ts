...
export class StatsView {
  ...
  private _emptyChart(canvasId: string, msg: string, data: number[]): boolean {
    if (data.some(v => v > 0)) return false;   // sa dane — rysuj normalnie
  ...
  private _renderMonthChart(mode: 'dist' | 'time'): void {
    ...
    if (this._emptyChart('svMonthChart', 'No activities this month', byDay)) return;
  ...
  private _renderYearChart(): void {
    ...
    if (this._emptyChart('svYearChart', 'No activities this year', byMonth)) return;
