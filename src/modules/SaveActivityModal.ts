...
export class SaveActivityModal {
  ...
  private _openSportPicker(onSelect: (sport: string) => void): void {
    ...
    const renderPicker = () => {
      ...
      all.forEach(s => {
        const c = (s as { category?: string }).category ?? 'Other';
      ...
      const sections = cats.map(c =>
        ...
        + byCat[c].map(s =>
            '<button data-pick="' + s.key + '" style="background:rgba(255,255,255,0.06);border:1.5px solid rgba(255,255,255,0.1);border-radius:12px;padding:12px 8px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:4px">'
      ...
      overlay.querySelectorAll<HTMLElement>('[data-pick]').forEach(btn => {
   ...
   */
  close(reason: 'saved' | 'discarded' | 'dismissed' = 'dismissed'): void {
    if (!this._el) return;
    ...
    if (reason === 'discarded') this._onCancel?.();
    else if (reason === 'dismissed') this._onDismiss?.();
  }
  ...
  private _confirmDiscard(): void {
    ...
    ov.addEventListener('click', e => {
      ...
      if (t.id === 'samDiscardGo') { ov.remove(); this.close('discarded'); }
      else if (t.id === 'samKeep' || e.target === ov) ov.remove();
    });
  ...
  private _bindEvents(): void {
    ...
    el.addEventListener('click', e => { if (e.target === el) this.close('dismissed'); });
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape') this.close('dismissed'); }, { once: true });
    ...
    // Desc char counter
    const samDescEl   = el.querySelector<HTMLTextAreaElement>('#samDesc');
    const samDescCount = el.querySelector<HTMLElement>('#samDescCount');
    samDescEl?.addEventListener('input', () => {
      if (samDescCount) samDescCount.textContent = `${samDescEl.value.length}/300`;
    });
    ...
    handle.addEventListener('touchend', e => {
      ...
      if (e.changedTouches[0].clientY - this._touchStartY > 100) this.close('dismissed');
