...
export async function restoreAccountByCode(code: string): Promise<string | null> {
  try {
...
export async function showRecoveryCodeModal(userId: string): Promise<void> {
  ...
  modal.querySelector('#recoveryCodeCopy')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const btn = modal.querySelector<HTMLButtonElement>('#recoveryCodeCopy')!;
...
function _showRestorePanel(modal: HTMLElement, resolve: () => void, backToNameSetup = true): void {
  ...
  const input = card.querySelector<HTMLInputElement>('#restoreCodeInput')!;
  const status = card.querySelector<HTMLElement>('#restoreStatus')!;
  input.focus();

  card.querySelector('#restoreBack')?.addEventListener('click', () => {
    modal.remove();
    if (backToNameSetup) void showNameModal();
    else resolve();
  });
  ...
  const doRestore = async () => {
    ...
    const btn = card.querySelector<HTMLButtonElement>('#restoreSubmit')!;
    btn.disabled = true;
  ...
  input.addEventListener('keydown', e => { if (e.key === 'Enter') void doRestore(); });
}
...
export function showRestoreAccountModal(): void {
