...
function _renderFull(
  ...
  void (async () => {
    ...
    if (!prosby.some(x => x.userId === profile.userId)) return;

    const bar = sheet.querySelector<HTMLElement>('#ppReqBar');
    if (!bar) return;
    bar.style.display = '';
    ...
    const decyduj = async (akceptuj: boolean): Promise<void> => {
      ...
      if (await decyzjaProsby(profile.userId, akceptuj)) {
        ...
        if (akceptuj) void openPublicProfile(profile.userId);
      } else {
