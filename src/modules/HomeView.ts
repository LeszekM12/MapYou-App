...
export class HomeView {
  ...
  private async _buildReelsBar(): Promise<HTMLElement | null> {
    ...
    if (!myReels && others.length === 0) return null;
    ...
    const profile = _profileRaw ? JSON.parse(_profileRaw) as { avatarB64?: string | null; name?: string } : { avatarB64: null, name: 'Me' };
    const myAvatarHtml = (profile as { avatarB64?: string | null }).avatarB64
      ? `<img src="${(profile as { avatarB64: string }).avatarB64}" class="home-reel-avatar__img" alt="avatar"/>`
      ...
      <div class="home-reel-creator__tools" id="reelCreatorTools" style="display:none">
        <div class="home-reel-creator__text-tools">
          ...
          <div class="home-reel-creator__colors" id="reelColors">
            ${['#ffffff','#000000','#ff3b30','#00c46a','#ffcc00','#007aff'].map(c =>
              `<button class="home-reel-creator__color-btn" data-color="${c}" style="background:${c}"></button>`
            ).join('')}
          </div>
