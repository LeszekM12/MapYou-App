...
export class HomeView {
  ...
  private async _openReelCreator(): Promise<void> {
    ...
    interface SLayer { id: string; kind: 'emoji' | 'location' | 'time' | 'hashtag' | 'mention'; text: string; x: number; y: number; scale: number; }
    ...
    overlay.querySelector('#reelFontPicker')?.addEventListener('click', () => {
      ...
      const lbl = overlay.querySelector<HTMLElement>('#reelFontLabel');
      if (lbl) lbl.style.fontFamily = FONTS[captionFontIdx].css;
    ...
    // Caption drag
    const _captionStyleCss = (style: string, color: string): string => {
      if (style === 'highlight') {
    ...
    const onMove = (x: number, y: number) => {
      ...
      for (const t of X_TARGETS) { if (Math.abs(px - t) < SNAP) { px = t; vGuide = t; break; } }
      for (const t of Y_TARGETS) { if (Math.abs(py - t) < SNAP) { py = t; hGuide = t; break; } }
    ...
    const wireSticker = (el: HTMLElement): void => {
      ...
      const applyStyle = (): void => { const L = layer(); if (L) { el.style.left = `${L.x}%`; el.style.top = `${L.y}%`; el.style.transform = `translate(-50%,-50%) scale(${L.scale})`; } };
      ...
      el.addEventListener('pointerdown', ev => {
        ...
        if (pts.size === 2) { const a = [...pts.values()]; pinchBase = { dist: _dist(a[0], a[1]), scale: layer()?.scale ?? 1 }; }
      });
      el.addEventListener('pointermove', ev => {
        if (!pts.has(ev.pointerId)) return;
        pts.set(ev.pointerId, { x: ev.clientX, y: ev.clientY });
        const L = layer(); if (!L) return;
        if (pts.size >= 2 && pinchBase) { const a = [...pts.values()]; L.scale = Math.max(0.4, Math.min(3, pinchBase.scale * (_dist(a[0], a[1]) / pinchBase.dist))); applyStyle(); return; }
        const rect = canvas.getBoundingClientRect();
      ...
      const end = (ev: PointerEvent): void => { pts.delete(ev.pointerId); if (pts.size < 2) pinchBase = null; if (pts.size === 0) clearGuides(); };
    ...
    const bakeFlat = async (): Promise<File | null> => {
      ...
      if (capText) {
        ...
        if (st === 'highlight') {
          ...
          _rr(ctx, x - w / 2 - padX, y - fs / 2 - padY, w + padX * 2, fs + padY * 2, fs * 0.2); ctx.fillStyle = captionColor; ctx.fill();
          ctx.fillStyle = dark ? '#000' : '#fff'; ctx.fillText(capText, x, y);
        } else if (st === 'neon') {
          ctx.save(); ctx.shadowColor = captionColor; ctx.shadowBlur = fs * 0.55; ctx.fillStyle = '#fff';
          ctx.fillText(capText, x, y); ctx.fillText(capText, x, y); ctx.restore();
        } else { ctx.fillStyle = captionColor; ctx.fillText(capText, x, y); }
      }
      // stickers
      for (const s of stickers) {
        ...
        if (s.kind === 'emoji') {
          ...
          ctx.fillText(s.text, x, y); continue;
        }
        ...
        _rr(ctx, x - bw / 2, y - bh / 2, bw, bh, bh / 2); ctx.fillStyle = '#ffffff'; ctx.fill(); ctx.restore();
        ctx.fillStyle = '#16181c'; ctx.fillText(label, x - bw / 2 + padX, y);
      }
    ...
    shareBtn.addEventListener('click', async () => {
      ...
      let captionMeta: string | null = captionEl.value || null;
      if (!isVideoMedia && (captionMeta || stickers.length > 0)) {
        const baked = await bakeFlat();
        if (baked) { fileToUpload = baked; captionMeta = null; }
      }
