...
function openCommentPanel(card: HTMLElement, actId: string): void {
  ...
  void fetch(`${BACKEND_URL}/feed/comments/${encodeURIComponent(realId)}`)
    ...
    .then((d: { data: Array<{authorName: string; text: string; createdAt: string}> }) => {
      ...
      if (actId || realId) SS.set((actId || realId) as string, { comments: n, fromServer: true });
...
export function buildPostCard(post: PostRecord, onRefresh: () => Promise<void> | void): HTMLElement {
  ...
  if (isLong) {
    ...
    card.querySelector(`#pmore-${post.id}`)?.addEventListener('click', e => {
      ...
      if (moreBtn) moreBtn.textContent = expanded ? 'mniej' : '…more';
    });
  }
...
export async function openActivityDetail(act: EnrichedActivity, isOwn: boolean, actId?: string): Promise<void> {
  ...
  const _recLikes    = rec._likeCount as number | undefined;
  ...
  const _recComments = rec._commentCount as number | undefined;
  ...
  // Load current like state + count, and the comment count
  {
    ...
    if (userId) {
      void fetch(`${BACKEND_URL}/feed/likes/batch?userId=${encodeURIComponent(userId)}&items=${encodeURIComponent(itemId)}`, { cache: 'no-store' })
        ...
        }).catch(() => { /* brak sieci — magazyn ma ostatni znany stan */ });
    }
...
function _openNotifPanel(): void {
  ...
  void wstawWierszProsb(panel.querySelector<HTMLElement>('#hnList'));
  ...
  const bindItems = (): void => {
    panel.querySelectorAll<HTMLElement>('.hn-item').forEach(item => {
      item.addEventListener('click', () => {
        ...
        if (n.type === 'follow_request') { close(); void pokazProsby(); return; }
        if (!n.target) return;
        close();
  ...
  void syncFromBackend(uid).then(() => {
    ...
    void wstawWierszProsb(listEl as HTMLElement);
    bindItems();
    markAllRead();
...
export class HomeView {
  ...
  private _buildGreeting(activityCount: number): HTMLElement {
    ...
    void liczbaProsb().then(n => {
      ...
      if (!bell) return;
      const suma = unread + n;
      let badge = bell.querySelector<HTMLElement>('.home-bell__badge');
  ...
  async render(): Promise<void> {
    ...
    const paintFeed = (feed: FeedItem[]): void => {
      ...
      for (const f of feed) {
        SS.linkIds([
          f.data.activityId as string | undefined,
          f.data.postId as string | undefined,
          f.data.id as string | undefined,
        ]);
      }
      feedList.innerHTML = '';
      ...
      if (!isOwn) {
        const autorId = (item.data.userId ?? null) as string | null;
        const autorNazwa = (item.data.authorName ?? '') as string;
        ...
        zglos.addEventListener('click', async e => {
          e.stopPropagation();
          const wynik = await menuCudzejTresci(
            item.kind === 'post' ? 'post' : 'activity',
            (item.data.postId ?? item.data.activityId ?? item.data.id) as string,
            autorId, autorNazwa);
          ...
          if (wynik === 'zablokowano') { card.remove(); void this.render(); }
          else if (wynik === 'ukryto') card.remove();
        });
        ...
        if (wiersz) wiersz.appendChild(zglos);
        else card.appendChild(zglos);
      }
      ...
      const pump = (): void => {
        ...
        if (mojePokolenie !== this._pokolenieMalowania) return;
        if (next >= feed.length) return;
        if (!feedList.isConnected) return;   // widok zniknal
  ...
  private _panelPodgladu: HTMLElement | null = null;

  private _pokazPodglad(kierunek: -1 | 1): void {
    this._usunPodglad();
    const lista = document.getElementById('homeFeedList');
    if (!lista?.parentElement) return;
    ...
    for (let i = 0; i < ile; i++) {
      const it = sasiad[i] as { kind: string; data: Record<string, unknown> };
      try {
        panel.appendChild(this._buildFriendFeedCard(
          it.kind, it.data, localStorage.getItem('mapyou_userId_profile') ?? ''));
  ...
  private _setupSectionSwipe(feedList: HTMLElement): void {
    ...
    feedList.addEventListener('touchmove', e => {
      ...
      if (this._panelPodgladu) this._panelPodgladu.style.transform = `translateX(${cl}px)`;
