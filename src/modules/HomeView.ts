...
export async function openActivityDetail(act: EnrichedActivity, isOwn: boolean): Promise<void> {
  ...
  ov.querySelector('#adShare')?.addEventListener('click', () => openSharePanel(ov.querySelector('.ad-sheet') as HTMLElement, full));
  ov.querySelector('#adLike')?.addEventListener('click', () => {
    const userId = localStorage.getItem('mapyou_userId_profile') ?? '';
    const span = ov.querySelector('#adLike span') as HTMLElement | null;
