...
class App {
  ...
  _initSettings(): void {
    ...
    const settingsTitle = document.querySelector<HTMLElement>('.settings-panel__title');
  ...
  async _restoreSessionIfAny(): Promise<void> {
    ...
    if (this._restorePendingSave()) return;
  ...
  async _openRoutesScreen(): Promise<void> {
    ...
    const renderCommunity = (filter = '') => {
      ...
      listEl.querySelectorAll<HTMLElement>('[data-cmode]').forEach(b =>
        b.addEventListener('click', () => {
          const m = b.dataset.cmode as 'near' | 'featured';
          if (m === communityMode) return;
          communityMode = m;
      ...
      listEl.querySelectorAll<HTMLElement>('[data-like]').forEach(btn =>
        btn.addEventListener('click', async e => {
          ...
          try {
            ...
            const j = await res.json() as { status: string; liked: boolean; count: number };
            if (j.status !== 'ok') return;
            r.liked = j.liked; r.likeCount = j.count;
            btn.classList.toggle('trk-route-like--on', j.liked);
            const cnt = btn.querySelector('.trk-route-like__count');
            if (cnt) cnt.textContent = String(j.count);
    ...
    const loadCommunity = async () => {
      ...
      try {
        ...
        if (communityMode === 'featured') qs.set('sort', 'featured');
  ...
  _finishWithActivity(
    ...
    // had happened.
    if (opts?.restored) openModal();
    else showGoodJobSplash(openModal);
  }
  ...
  _updateCreateUI(): void {
    ...
    document.getElementById('trkCreateLoop')?.classList.toggle('hidden', this.#createWaypoints.length > 0);
  ...
  async _autoLoop(km: number): Promise<void> {
    ...
    try {
      ...
      const res  = await fetch(`${BACKEND_URL}/loop?lat=${start[0]}&lng=${start[1]}&km=${km}&sport=${encodeURIComponent(this.#createSport)}&seed=${seed}`);
      if (res.ok) {
        const data = await res.json() as { status: string; coords?: Array<[number, number]>; distanceKm?: number };
  ...
  _renderGeneratedLoop(coords: Array<[number, number]>, km: number): void {
    if (!this.#map) return;
    this._createClear();
    ...
    this.#createCoords     = coords;
    this.#createDistanceKm = km;
    this.#createLine = L.polyline(coords, { color: '#ff5a1f', weight: 5, opacity: 0.95 }).addTo(this.#map);
    this._updateCreateUI();
    try { this.#map.fitBounds(this.#createLine.getBounds(), { padding: [40, 40] }); } catch { /* ignore */ }
  }

  async _autoLoopMapbox(start: [number, number], km: number): Promise<void> {
  ...
  _openAutoLoopDialog(): void {
    if (this.#createWaypoints.length > 0) return;                    // only when builder is empty
    ...
    ov.innerHTML = `<div class="trk-picker trk-publish">
      ...
      <div class="trk-publish__body">
        ...
        <p class="trk-publish__note">Generates a ${getSportLabel(this.#createSport)} loop from your location, snapped to roads. The actual distance may differ slightly — edit the points afterwards if needed.</p>
...
setTimeout(() => {
  ...
  if (localStorage.getItem(ASKED) === 'true') return;
  if (localStorage.getItem('mapyou_recovery_code')) return;
...
(function setupClubDeepLink(): void {
  ...
  const resolveCode = async (code: string): Promise<void> => {
    try {
      ...
      const d   = await res.json() as { status?: string; clubId?: string };
      if (res.ok && d.status === 'ok' && d.clubId) { openClub(d.clubId); return; }
  ...
  const handle = (url: string): boolean => {
    ...
    if (direct) { setTimeout(() => openClub(direct), 600); return true; }
    const code = grab(url, 'club');
    if (code) { setTimeout(() => void resolveCode(code), 600); return true; }
    return false;
  };
  ...
  navigator.serviceWorker?.addEventListener('message', e => {
    const data = (e as MessageEvent).data as { type?: string; url?: string } | undefined;
    if (data?.type === 'OPEN_CLUB') handle(String(data.url ?? ''));
  });
})();
